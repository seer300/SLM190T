/**
* @file		lpuart_normal_uart_IT_demo.c
* @brief    LPUART的普通UART(即芯片在低功耗模式下，LPUART不保持供电且不具备唤醒功能)中断收发Demo
...
/**
 * @brief LPUART的普通UART(即芯片在低功耗模式下，LPUART不保持供电且不具备唤醒功能)初始化函数，主要完成以下配置：
 * 		  1. LPUART的TXD、RXD、CTS、RTS引脚选择 @ref HAL_LPUART_GPIOStateTypeDef
 * 		  2. LPUART的波特率、位宽、奇偶检验模式、硬件流控的配置
 */
void LPUART_Normal_UART_Init(void)
{
    ...
    g_lpuart_used = 0; // LPUART在芯片低功耗模式下供电关闭，且不为低功耗模式唤醒源
}
