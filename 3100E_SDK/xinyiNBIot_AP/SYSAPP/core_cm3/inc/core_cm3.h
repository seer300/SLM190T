...
/**
 * @brief 读取某个中断的NVIC使能状态
 * @param IRQn NVIC中断号
 * @return 1:使能，0:禁能 
 */
static __INLINE uint32_t NVIC_GetEnableIRQ(IRQn_Type IRQn)
{
    return ((( NVIC->ISER[((uint32_t)(IRQn) >> 5)] & (1 << ((uint32_t)(IRQn)&0x1F)) ) != 0) ? 1 : 0);
}
  ...
  static __INLINE void NVIC_SetPriority(IRQn_Type IRQn, uint32_t priority)
  {
    ...
    {
#if(NVIC_INT_NEST_DISABLE == 1)
    #if 1//(USER_SPECIAL == 1)
        if(IRQn == WDT_IRQn)
        {
          priority = 0;
        }
        else if(IRQn == LPTIM_IRQn)
        {
          priority = 1;        
        }
        else
  ...
  static __INLINE void NVIC_SetVectorTable(uint32_t *NVIC_Vectors)
  {
