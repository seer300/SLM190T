...
#define ADVANCE_OPT(o,e,step) if ((e) < step) {           \
    coap_log(LIBCOAP_LOG_DEBUG, "cannot advance opt past end\n"); \
    return 0;                                             \
  } else {                                                \
    (e) -= step;                                          \
    (o) = ((o)) + step;                                   \
...
size_t
coap_opt_parse(const coap_opt_t *opt, size_t length, libcoap_coap_option_t *result) {

  const coap_opt_t *opt_start = opt; /* store where parsing starts  */

  xy_assert(opt); xy_assert(result);
  ...
  case 15:
    if (*opt != COAP_PAYLOAD_START) {
      coap_log(LIBCOAP_LOG_DEBUG, "ignored reserved option delta 15\n");
    }
    return 0;
  case 14:
    ...
    if (result->delta < 269) {
      coap_log(LIBCOAP_LOG_DEBUG, "delta too large\n");
      return 0;
    }
    /* fall through */
  case 13:
  ...
  case 15:
    coap_log(LIBCOAP_LOG_DEBUG, "found reserved option length 15\n");
    return 0;
  case 14:
    /* Handle two-byte value: First, the MSB + 269 is stored as delta value.
     * After that, the option pointer is advanced to the LSB which is handled
  ...
  if (length < result->length) {
    coap_log(LIBCOAP_LOG_DEBUG, "invalid option length\n");
    return 0;
  }
...
coap_opt_delta(const coap_opt_t *opt) {
  ...
  switch (n) {
  case 15: /* error */
    coap_log(LIBCOAP_LOG_WARNING, "coap_opt_delta: illegal option delta\n");
...
coap_opt_length(const coap_opt_t *opt) {
  ...
  switch (*opt & 0xf0) {
  case 0xf0:
    coap_log(LIBCOAP_LOG_DEBUG, "illegal option delta\n");
    return 0;
  case 0xe0:
    ++opt;
    /* fall through */
  ...
  switch (length) {
  case 0x0f:
    coap_log(LIBCOAP_LOG_DEBUG, "illegal option length\n");
    return 0;
  case 0x0e:
    length = (*opt++ << 8) + 269;
    /* fall through */
...
coap_opt_setheader(coap_opt_t *opt, size_t maxlen,
  ...
  if (delta < 13) {
    ...
    if (maxlen < 2) {
      coap_log(LIBCOAP_LOG_DEBUG, "insufficient space to encode option delta %d\n",
               delta);
      return 0;
    }
  ...
  if (length < 13) {
    ...
    if (maxlen < skip + 2) {
      coap_log(LIBCOAP_LOG_DEBUG, "insufficient space to encode option length %zu\n",
               length);
      return 0;
    }
...
coap_opt_encode(coap_opt_t *opt, size_t maxlen, uint16_t delta,
  ...
  if (!l) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_opt_encode: cannot set option header\n");
    return 0;
  }
  ...
  if (maxlen < length) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_opt_encode: option too large for buffer\n");
    return 0;
  }
...
coap_new_optlist(uint16_t number,
  ...
  if (node) {
    ...
    coap_log(LIBCOAP_LOG_WARNING, "coap_new_optlist: malloc failure\n");
  }
...
coap_insert_optlist(coap_optlist_t **head, coap_optlist_t *node) {
  if (!node) {
    coap_log(LIBCOAP_LOG_DEBUG, "optlist not provided\n");
  } else {
