...
static coap_log_t maxlog = LIBCOAP_LOG_WARNING;        /* default maximum log level */
...
msg_type_string(uint16_t t) {
  const char *types[] = { "CON", "NON", "ACK", "RST", "???" };
...
msg_code_string(uint16_t c) {
  const char *methods[] = { "0.00", "GET", "POST", "PUT", "DELETE",
                                   "FETCH", "PATCH", "iPATCH" };
  const char *signals[] = { "7.00", "CSM", "Ping", "Pong", "Release",
                                   "Abort" };
  char buf[5];
...
msg_option_string(uint8_t code, uint16_t option_type) {
  ...
  struct option_desc_t options[] = {
    { LIBCOAP_COAP_OPTION_IF_MATCH, "If-Match" },
    { LIBCOAP_COAP_OPTION_URI_HOST, "Uri-Host" },
    { LIBCOAP_COAP_OPTION_ETAG, "ETag" },
    { LIBCOAP_COAP_OPTION_IF_NONE_MATCH, "If-None-Match" },
    { LIBCOAP_COAP_OPTION_OBSERVE, "Observe" },
    { LIBCOAP_COAP_OPTION_URI_PORT, "Uri-Port" },
    { LIBCOAP_COAP_OPTION_LOCATION_PATH, "Location-Path" },
    { LIBCOAP_COAP_OPTION_URI_PATH, "Uri-Path" },
    { LIBCOAP_COAP_OPTION_CONTENT_FORMAT, "Content-Format" },
    { LIBCOAP_COAP_OPTION_MAXAGE, "Max-Age" },
    { LIBCOAP_COAP_OPTION_URI_QUERY, "Uri-Query" },
    { LIBCOAP_COAP_OPTION_ACCEPT, "Accept" },
    { LIBCOAP_COAP_OPTION_LOCATION_QUERY, "Location-Query" },
    { LIBCOAP_COAP_OPTION_BLOCK2, "Block2" },
    { LIBCOAP_COAP_OPTION_BLOCK1, "Block1" },
    { LIBCOAP_COAP_OPTION_PROXY_URI, "Proxy-Uri" },
    { LIBCOAP_COAP_OPTION_PROXY_SCHEME, "Proxy-Scheme" },
    { LIBCOAP_COAP_OPTION_SIZE1, "Size1" },
    { LIBCOAP_COAP_OPTION_SIZE2, "Size2" },
    { LIBCOAP_COAP_OPTION_NORESPONSE, "No-Response" }
  };

  struct option_desc_t options_csm[] = {
    { COAP_SIGNALING_OPTION_MAX_MESSAGE_SIZE, "Max-Message-Size" },
    { COAP_SIGNALING_OPTION_BLOCK_WISE_TRANSFER, "Block-wise-Transfer" }
  };

  struct option_desc_t options_pingpong[] = {
    { COAP_SIGNALING_OPTION_CUSTODY, "Custody" }
  };

  struct option_desc_t options_release[] = {
    { COAP_SIGNALING_OPTION_ALTERNATIVE_ADDRESS, "Alternative-Address" },
    { COAP_SIGNALING_OPTION_HOLD_OFF, "Hold-Off" }
  };

  struct option_desc_t options_abort[] = {
...
#define COAP_DO_SHOW_OUTPUT_LINE           \
 do {                                      \
   if (use_fprintf_for_show_pdu) {         \
     xy_printf(0,XYAPP, WARN_LOG,"%s", outbuf); \
   }                                       \
   else {                                  \
...
coap_show_pdu(coap_log_t level, const coap_pdu_t *pdu) {
  ...
  coap_mutex_t static_show_pdu_mutex = COAP_MUTEX_INITIALIZER;
  unsigned char buf[min(COAP_DEBUG_BUF_SIZE, 1024)] = {0x00}; /* need some space for output creation */
  char outbuf[COAP_DEBUG_BUF_SIZE] = {0x00};
  ...
  while ((option = coap_option_next(&opt_iter))) {
    ...
    default:
      ...
      if (opt_iter.type == LIBCOAP_COAP_OPTION_URI_PATH ||
          opt_iter.type == LIBCOAP_COAP_OPTION_PROXY_URI ||
          opt_iter.type == LIBCOAP_COAP_OPTION_URI_HOST ||
          opt_iter.type == LIBCOAP_COAP_OPTION_LOCATION_PATH ||
          opt_iter.type == LIBCOAP_COAP_OPTION_LOCATION_QUERY ||
          opt_iter.type == LIBCOAP_COAP_OPTION_URI_QUERY) {
        encode = 0;
      } else {
        encode = 1;
      }
...
coap_log_impl(coap_log_t level, const char *format, ...) {
  ...
  if (log_handler) {
#if COAP_CONSTRAINED_STACK
    coap_mutex_t static_log_mutex = COAP_MUTEX_INITIALIZER;
    char message[COAP_DEBUG_BUF_SIZE] = {0x00};
#else /* ! COAP_CONSTRAINED_STACK */
...
int coap_debug_set_packet_loss(const char *loss_level) {
  ...
  if (*end == '%') {
    ...
    coap_log(LIBCOAP_LOG_DEBUG, "packet loss level set to %d%%\n", n);
  } else {
    if (n <= 0)
      return 0;
    while (i < 10) {
