...
match(const coap_str_const_t *text, const coap_str_const_t *pattern, int match_prefix,
  ...
  xy_assert(text); xy_assert(pattern);
...
#define MATCH_SUBSTRING 0x04
  ...
  if (query_filter) {
    ...
    if (resource_param.length < coap_opt_length(query_filter)) {
      ...
      xy_assert((resource_param.length + 1) <= coap_opt_length(query_filter));
...
coap_resource_init(coap_str_const_t *uri_path, int flags) {
  ...
  if (r) {
    ...
    coap_log(LIBCOAP_LOG_DEBUG, "coap_resource_init: no memory left\n");
  }
...
coap_resource_unknown_init(coap_method_handler_t put_handler) {
  ...
  if (r) {
    ...
    coap_log(LIBCOAP_LOG_DEBUG, "coap_resource_unknown_init: no memory left\n");
  }
...
coap_add_attr(coap_resource_t *resource,
  ...
  if (attr) {
    ...
    coap_log(LIBCOAP_LOG_DEBUG, "coap_add_attr: no memory left\n");
  }
...
coap_register_handler(coap_resource_t *resource,
  ...
  xy_assert(resource);
  xy_assert(method > 0 && (size_t)(method-1) < sizeof(resource->handler)/sizeof(coap_method_handler_t));
...
coap_add_observer(coap_resource_t *resource,
  ...
  coap_log(LIBCOAP_LOG_DEBUG, "create new subscription\n");
...
coap_delete_observer(coap_resource_t *resource, coap_session_t *session,
  ...
  if ( s && coap_get_log_level() >= LIBCOAP_LOG_DEBUG ) {
    char outbuf[2 * 8 + 1] = "";
    unsigned int i;
    for ( i = 0; i < s->token_length; i++ )
      snprintf( &outbuf[2 * i], 3, "%02x", s->token[i] );
    coap_log(LIBCOAP_LOG_DEBUG, "removed observer tid %s\n", outbuf);
  }
...
coap_notify_observers(coap_context_t *context, coap_resource_t *r) {
  ...
  if (r->observable && (r->dirty || r->partiallydirty)) {
    ...
    xy_assert(h);                /* we do not allow subscriptions if no
...
#define INET6_ADDRSTRLEN 40
          ...
          if (coap_print_addr(&obs->session->addr_info.remote,
            ...
            coap_log(LIBCOAP_LOG_DEBUG, "** removed observer %s\n", addr);
        }
