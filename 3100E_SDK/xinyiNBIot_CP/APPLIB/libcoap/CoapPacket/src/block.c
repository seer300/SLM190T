...
coap_write_block_opt(coap_block_t *block, uint16_t type,
  ...
  if (data_length <= start) {
    coap_log(LIBCOAP_LOG_DEBUG, "illegal block requested\n");
    return -2;
  }

  xy_assert(pdu->max_size > 0);
...
coap_add_data_blocked_response(coap_resource_t *resource,
   ...
   */
  if (request) {
    if (coap_get_block(request, LIBCOAP_COAP_OPTION_BLOCK2, &block2)) {
      block2_requested = 1;
      if (length <= (block2.num << (block2.szx + 4))) {
        coap_log(LIBCOAP_LOG_DEBUG, "Illegal block requested (%d > last = %zu)\n",
                 block2.num,
                 length >> (block2.szx + 4));
  ...
  coap_add_option(response, LIBCOAP_COAP_OPTION_ETAG, sizeof(etag), etag);

  if ((block2.num == 0) && subscription) {
    coap_add_option(response, LIBCOAP_COAP_OPTION_OBSERVE,
                    coap_encode_var_safe(buf, sizeof (buf),
                                         resource->observe),
                    buf);
  }

  coap_add_option(response, LIBCOAP_COAP_OPTION_CONTENT_TYPE,
                  coap_encode_var_safe(buf, sizeof(buf),
                                       media_type),
                  buf);
  ...
  if (block2_requested) {
    ...
    res = coap_write_block_opt(&block2, LIBCOAP_COAP_OPTION_BLOCK2, response,
                               length);
    ...
    case -1:                        /* should really not happen */
        xy_assert(0);
        /* fall through if xy_assert is a no-op */
  ...
  if (!coap_add_data(response, length, data)) {
    ...
    block2.num = 0;
    block2.szx = 6;
    coap_write_block_opt(&block2, LIBCOAP_COAP_OPTION_BLOCK2, response,
                         length);
