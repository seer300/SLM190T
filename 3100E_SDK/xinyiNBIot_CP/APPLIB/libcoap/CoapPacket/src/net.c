...
coap_new_node(void) {
  ...
  if (!node) {
    coap_log(LIBCOAP_LOG_WARNING, "coap_new_node: malloc failed\n");
    return NULL;
  }
...
int coap_context_set_psk(coap_context_t *ctx,
  ...
  if (hint) {
    ...
    if (ctx->psk_hint) {
      ...
      coap_log(LIBCOAP_LOG_ERR, "No memory to store PSK hint\n");
      return 0;
    }
  }
  ...
  if (key && key_len > 0) {
    ...
    if (ctx->psk_key) {
      ...
      coap_log(LIBCOAP_LOG_ERR, "No memory to store PSK key\n");
      return 0;
    }
  }
...
int coap_context_set_pki(coap_context_t *ctx,
  ...
  if (setup_data->version != COAP_DTLS_PKI_SETUP_VERSION) {
    coap_log(LIBCOAP_LOG_ERR, "coap_context_set_pki: Wrong version of setup_data\n");
    return 0;
  }
  if (coap_dtls_is_supported()) {
...
coap_new_context(
  ...
  if (c->epfd == -1) {
    coap_log(LIBCOAP_LOG_ERR, "coap_new_context: Unable to epoll_create: %s (%d)\n",
  ...
  if (c->epfd != -1) {
    ...
    if (c->eptimerfd == -1) {
      coap_log(LIBCOAP_LOG_ERR, "coap_new_context: Unable to timerfd_create: %s (%d)\n",
               coap_socket_strerror(),
               errno);
    }
    else {
...
coap_option_check_critical(coap_context_t *ctx,
  ...
  while (coap_option_next(&opt_iter)) {
    ...
    if (opt_iter.type & 0x01) {
      ...
      case LIBCOAP_COAP_OPTION_PROXY_SCHEME:
      ...
      default:
        if (coap_option_filter_get(ctx->known_options, opt_iter.type) <= 0) {
          coap_log(LIBCOAP_LOG_DEBUG, "unknown critical option %d\n", opt_iter.type);
          ok = 0;
...
coap_session_send_pdu(coap_session_t *session, coap_pdu_t *pdu) {
  ...
  coap_show_pdu(LIBCOAP_LOG_DEBUG, pdu);
...
coap_send_pdu(coap_session_t *session, coap_pdu_t *pdu, coap_queue_t *node) {
  ...
  *        must not be dropped. */
  ...
  if (pdu->type == COAP_MESSAGE_CON &&
    ...
    coap_log(LIBCOAP_LOG_ERR, "Multicast requests cannot be Confirmable (RFC7252 8.1)\n");
    return -1;
  }
...
coap_wait_ack(coap_context_t *context, coap_session_t *session,
  ...
  */
  ...
  {                            /* (re-)initialize retransmission timer */
    ...
    xy_assert(nextpdu);                /* we have just inserted a node */
  ...
  coap_log(LIBCOAP_LOG_DEBUG, "** %s: tid=%d: added to retransmit queue (%ums)\n",
...
coap_send(coap_session_t *session, coap_pdu_t *pdu) {
  ...
  if (!node) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_wait_ack: insufficient memory\n");
    goto error;
  }
...
coap_retransmit(coap_context_t *context, coap_queue_t *node) {
  ...
  if (node->retransmit_cnt < node->session->max_retransmit) {
    ...
    coap_log(LIBCOAP_LOG_DEBUG, "** %s: tid=%d: retransmission #%d\n",
...
#ifndef WITH_CONTIKI
  coap_log(LIBCOAP_LOG_DEBUG, "** %s: tid=%d: give up after %d attempts\n",
           coap_session_str(node->session), node->id, node->retransmit_cnt);
#endif
...
coap_write_session(coap_context_t *ctx, coap_session_t *session, coap_tick_t now) {
  (void)ctx;
  xy_assert(session->sock.flags & COAP_SOCKET_CONNECTED);
  ...
  while (session->delayqueue) {
    ...
    xy_assert(session->partial_write < q->pdu->used_size + q->pdu->hdr_size);
    switch (session->proto) {
      case COAP_PROTO_TCP:
...
coap_read_session(coap_context_t *ctx, coap_session_t *session, coap_tick_t now) {
  ...
  xy_assert(session->sock.flags & (COAP_SOCKET_CONNECTED | COAP_SOCKET_MULTICAST));
  ...
  if (COAP_PROTO_NOT_RELIABLE(session->proto)) {
    ...
    if (bytes_read < 0) {
      if (bytes_read == -2)
        ...
        coap_log(LIBCOAP_LOG_WARNING, "*  %s: read error\n",
                 coap_session_str(session));
    } else if (bytes_read > 0) {
      session->last_rx_tx = now;
      memcpy(&session->addr_info, &packet->addr_info,
             sizeof(session->addr_info));
      coap_log(LIBCOAP_LOG_DEBUG, "*  %s: received %zd bytes\n",
...
coap_read_endpoint(coap_context_t *ctx, coap_endpoint_t *endpoint, coap_tick_t now) {
  ...
  xy_assert(COAP_PROTO_NOT_RELIABLE(endpoint->proto));
  xy_assert(endpoint->sock.flags & COAP_SOCKET_BOUND);
  ...
  if (bytes_read < 0) {
    coap_log(LIBCOAP_LOG_WARNING, "*  %s: read failed\n", coap_endpoint_str(endpoint));
  } else if (bytes_read > 0) {
...
coap_handle_dgram(coap_context_t *ctx, coap_session_t *session,
  ...
  xy_assert(COAP_PROTO_NOT_RELIABLE(session->proto));
  ...
  if (!coap_pdu_parse(session->proto, msg, msg_len, pdu)) {
    coap_log(LIBCOAP_LOG_WARNING, "discard malformed PDU\n");
    goto error;
  }
...
coap_remove_from_queue(coap_queue_t **queue, coap_session_t *session, coap_tid_t id, coap_queue_t **node) {
  ...
  if (session == (*queue)->session && id == (*queue)->id) { /* found transaction */
    ...
    *queue = (*queue)->next;
    ...
    (*node)->next = NULL;
    coap_log(LIBCOAP_LOG_DEBUG, "** %s: tid=%d: removed\n",
...
coap_new_error_response(coap_pdu_t *request, unsigned char code,
  ...
  coap_option_clrb(opts, LIBCOAP_COAP_OPTION_CONTENT_TYPE); /* we do not want this */
  ...
  if (response) {
    ...
    if (!coap_add_token(response, request->token_length,
      request->token)) {
      coap_log(LIBCOAP_LOG_DEBUG, "cannot add token to error response\n");
...
get_wkc_len(coap_context_t *context, coap_opt_t *query_filter) {
  ...
  if (coap_print_wellknown(context, buf, &len, UINT_MAX, query_filter)
    ...
    coap_log(LIBCOAP_LOG_WARNING, "cannot determine length of /.well-known/core\n");
    return 0;
  }

  coap_log(LIBCOAP_LOG_DEBUG, "get_wkc_len: coap_print_wellknown() returned %zu\n", len);
...
coap_wellknown_response(coap_context_t *context, coap_session_t *session,
  ...
  if (!resp) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_wellknown_response: cannot create PDU\n");
    return NULL;
  }

  if (!coap_add_token(resp, request->token_length, request->token)) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_wellknown_response: cannot add token\n");
    goto error;
  }

  query_filter = coap_check_option(request, LIBCOAP_COAP_OPTION_URI_QUERY, &opt_iter);
  wkc_len = get_wkc_len(context, query_filter);

  /* The value of some resources is undefined and get_wkc_len will return 0.*/
  if (wkc_len == 0) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_wellknown_response: undefined resource\n");
  ...
  if (coap_get_block(request, LIBCOAP_COAP_OPTION_BLOCK2, &block)) {
    coap_log(LIBCOAP_LOG_DEBUG, "create block\n");
  ...
  if (resp->max_size && resp->max_size <= resp->used_size + 8) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_wellknown_response: insufficient storage space\n");
    goto error;
  }
  ...
  if (!need_block2 && resp->max_size && resp->max_size - resp->used_size < wkc_len + 1) {
    xy_assert(resp->used_size <= resp->max_size);
  ...
  xy_assert(coap_encode_var_safe(buf, sizeof(buf),
    COAP_MEDIATYPE_APPLICATION_LINK_FORMAT) == 1);
  coap_add_option(resp, LIBCOAP_COAP_OPTION_CONTENT_FORMAT,
  ...
  if (need_block2) {
    if (coap_write_block_opt(&block, LIBCOAP_COAP_OPTION_BLOCK2, resp, wkc_len) < 0) {
      coap_log(LIBCOAP_LOG_DEBUG,
  ...
  if (!data) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_wellknown_response: coap_add_data failed\n" );
    goto error;
  }

  result = coap_print_wellknown(context, data, &len, offset, query_filter);
  if ((result & COAP_PRINT_STATUS_ERROR) != 0) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_print_wellknown failed\n");
    goto error;
  }
...
no_response(coap_pdu_t *request, coap_pdu_t *response) {
  ...
  if (COAP_RESPONSE_CLASS(response->code) > 0) {
    nores = coap_check_option(request, LIBCOAP_COAP_OPTION_NORESPONSE, &opt_iter);
...
handle_request(coap_context_t *context, coap_session_t *session, coap_pdu_t *pdu) {
  ...
  if (context->response_handler) {
    context->response_handler(context, session, NULL, pdu, pdu->tid);
    return;
  }
  ...
  if ((resource == NULL) || (resource->is_unknown == 1)) {
    ...
    if (coap_string_equal(uri_path, &coap_default_uri_wellknown)) {
      ...
      if (pdu->code == COAP_REQUEST_GET) { /* GET */
        coap_log(LIBCOAP_LOG_INFO, "create default response for %s\n",
                 COAP_DEFAULT_URI_WELLKNOWN);
        response = coap_wellknown_response(context, session, pdu);
      } else {
        coap_log(LIBCOAP_LOG_DEBUG, "method not allowed for .well-known/core\n");
       ...
       */
      coap_log(LIBCOAP_LOG_DEBUG, "request for unknown resource '%*.*s',"
                          " return 2.02\n",
                          (int)uri_path->length,
                          (int)uri_path->length,
                          uri_path->s);
      ...
      coap_log(LIBCOAP_LOG_DEBUG, "request for unknown resource '%*.*s', return 4.04\n",
    ...
    if (!resource) {
      if (response && (no_response(pdu, response) != RESPONSE_DROP)) {
        if (coap_send(session, response) == COAP_INVALID_TID)
          coap_log(LIBCOAP_LOG_WARNING, "cannot send response for transaction %u\n",
                   pdu->tid);
      } else {
        coap_delete_pdu(response);
      }
  ...
  if (h) {
    ...
    int owns_query = 1;
     coap_log(LIBCOAP_LOG_DEBUG, "call custom handler for resource '%*.*s'\n",
    ...
    if (coap_add_token(response, pdu->token_length, pdu->token)) {
      ...
      if (resource->observable) {
        observe = coap_check_option(pdu, LIBCOAP_COAP_OPTION_OBSERVE, &opt_iter);
        if (observe) {
          observe_action =
          ...
          if ((observe_action & COAP_OBSERVE_CANCEL) == 0) {
            ...
            if (coap_get_block(pdu, LIBCOAP_COAP_OPTION_BLOCK2, &block2)) {
              has_block2 = 1;
            }
      ...
      if (respond != RESPONSE_DROP) {
        ...
        if ((respond == RESPONSE_SEND)
          ...
          if (coap_send(session, response) == COAP_INVALID_TID)
            coap_log(LIBCOAP_LOG_DEBUG, "cannot send response for message %d\n",
                     pdu->tid);
        } else {
          coap_delete_pdu(response);
        }
      ...
      coap_log(LIBCOAP_LOG_WARNING, "cannot generate response\r\n");
    }
  } else {
    if (coap_string_equal(uri_path, &coap_default_uri_wellknown)) {
      /* request for .well-known/core */
      coap_log(LIBCOAP_LOG_DEBUG, "create default response for %s\n",
               COAP_DEFAULT_URI_WELLKNOWN);
      response = coap_wellknown_response(context, session, pdu);
      coap_log(LIBCOAP_LOG_DEBUG, "have wellknown response %p\n", (void *)response);
    } else
      response = coap_new_error_response(pdu, COAP_RESPONSE_CODE(405),
        opt_filter);
    ...
    if (response && (no_response(pdu, response) != RESPONSE_DROP)) {
      if (coap_send(session, response) == COAP_INVALID_TID)
        coap_log(LIBCOAP_LOG_DEBUG, "cannot send response for transaction %d\n",
                 pdu->tid);
    } else {
      coap_delete_pdu(response);
    }
...
#define INET6_ADDRSTRLEN 40
    ...
    if (coap_print_addr(remote, addr, INET6_ADDRSTRLEN+8) &&
      ...
      coap_log(LIBCOAP_LOG_DEBUG, "** received %d bytes from %s on interface %s:\n",
  ...
  switch (pdu->type) {
    ...
    case COAP_MESSAGE_CON:        /* check for unknown critical options */
      if (coap_option_check_critical(context, pdu, opt_filter) == 0) {
        ...
        if (!response) {
          coap_log(LIBCOAP_LOG_WARNING,
                   "coap_dispatch: cannot create error response\n");
        } else {
          if (coap_send(session, response) == COAP_INVALID_TID)
            coap_log(LIBCOAP_LOG_WARNING, "coap_dispatch: error sending response\n");
        }
  ...
  if (COAP_PDU_IS_SIGNALING(pdu))
    ...
    coap_log(LIBCOAP_LOG_DEBUG, "dropped message with invalid code (%d.%02d)\n",
...
coap_handle_event(coap_context_t *context, coap_event_t event, coap_session_t *session) {
  coap_log(LIBCOAP_LOG_DEBUG, "***EVENT: 0x%04x\n", event);
...
PROCESS_THREAD(coap_retransmit_process, ev, data) {
  ...
  coap_log(LIBCOAP_LOG_DEBUG, "Started retransmit process\n");
