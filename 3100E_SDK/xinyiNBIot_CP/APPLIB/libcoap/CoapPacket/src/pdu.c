...
coap_pdu_clear(coap_pdu_t *pdu, size_t size) {
  xy_assert(pdu);
  xy_assert(pdu->token);
  xy_assert(pdu->max_hdr_size >= COAP_PDU_MAX_UDP_HEADER_SIZE);
...
coap_pdu_init(uint8_t type, uint8_t code, uint16_t tid, size_t size) {
  ...
  xy_assert(size <= COAP_MAX_MESSAGE_SIZE_TCP16 + 4);
...
coap_pdu_resize(coap_pdu_t *pdu, size_t new_size) {
  if (new_size > pdu->alloc_size) {
    ...
    if (pdu->max_size && new_size > pdu->max_size) {
      coap_log(LIBCOAP_LOG_WARNING, "coap_pdu_resize: pdu too big\n");
      return 0;
    }
#if !defined(WITH_LWIP) && !defined(WITH_CONTIKI)
    if (pdu->data != NULL) {
      xy_assert(pdu->data > pdu->token);
    ...
    if (new_hdr == NULL) {
      coap_log(LIBCOAP_LOG_WARNING, "coap_pdu_resize: realloc failed\n");
      return 0;
    }
...
coap_add_option(coap_pdu_t *pdu, uint16_t type, size_t len, const uint8_t *data) {
  ...
  if (!optsize) {
    coap_log(LIBCOAP_LOG_WARNING, "coap_add_option: cannot add option\n");
    /* error */
    return 0;
  } else {
    pdu->max_delta = type;
...
next_option_safe(coap_opt_t **optp, size_t *length) {
  libcoap_coap_option_t option;
  size_t optsize;

  xy_assert(optp); xy_assert(*optp);
  xy_assert(length);

  optsize = coap_opt_parse(*optp, *length, &option);
  if (optsize) {
    xy_assert(optsize <= *length);
...
coap_pdu_parse_size(coap_proto_t proto,
                    ...
                    size_t length) {
  xy_assert(data);
  xy_assert(proto == COAP_PROTO_TCP || proto == COAP_PROTO_TLS);
  xy_assert(coap_pdu_parse_header_size(proto, data) <= length );
...
coap_pdu_parse_header(coap_pdu_t *pdu, coap_proto_t proto) {
  ...
  if (proto == COAP_PROTO_UDP || proto == COAP_PROTO_DTLS) {
    xy_assert(pdu->hdr_size == 4);
    if ((hdr[0] >> 6) != COAP_DEFAULT_VERSION) {
      coap_log(LIBCOAP_LOG_DEBUG, "coap_pdu_parse: UDP version not supported\n");
      return 0;
    }
    ...
    xy_assert(pdu->hdr_size >= 2 && pdu->hdr_size <= 6);
    pdu->type = COAP_MESSAGE_CON;
    pdu->token_length = hdr[0] & 0x0f;
    pdu->code = hdr[pdu->hdr_size-1];
    pdu->tid = 0;
  } else {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_pdu_parse: unsupported protocol\n");
    return 0;
  }
  if (pdu->token_length > pdu->alloc_size) {
    /* Invalid PDU provided - not wise to xy_assert here though */
    coap_log(LIBCOAP_LOG_DEBUG, "coap_pdu_parse: PDU header token size broken\n");
...
coap_pdu_parse_opt(coap_pdu_t *pdu) {
  ...
  if (pdu->code == 0) {
    if (pdu->used_size != 0 || pdu->token_length) {
      coap_log(LIBCOAP_LOG_DEBUG, "coap_pdu_parse: empty message is not empty\n");
      return 0;
    }
  }

  if (pdu->token_length > pdu->used_size || pdu->token_length > 8) {
    coap_log(LIBCOAP_LOG_DEBUG, "coap_pdu_parse: invalid Token\n");
    return 0;
  }
  ...
  if (pdu->code == 0) {
    ...
    while (length > 0 && *opt != COAP_PAYLOAD_START) {
      if ( !next_option_safe( &opt, (size_t *)&length ) ) {
        coap_log(LIBCOAP_LOG_DEBUG, "coap_pdu_parse: missing payload start code\n");
        return 0;
      }
    }

    if (length > 0) {
      xy_assert(*opt == COAP_PAYLOAD_START);
      opt++; length--;
...
coap_pdu_encode_header(coap_pdu_t *pdu, coap_proto_t proto) {
  if (proto == COAP_PROTO_UDP || proto == COAP_PROTO_DTLS) {
    ...
    size_t len;
    xy_assert(pdu->used_size >= pdu->token_length);
    if (pdu->used_size < pdu->token_length) {
      coap_log(LIBCOAP_LOG_WARNING, "coap_pdu_encode_header: corrupted PDU\n");
      return 0;
    }
    len = pdu->used_size - pdu->token_length;
    if (len <= COAP_MAX_MESSAGE_SIZE_TCP0) {
      xy_assert(pdu->max_hdr_size >= 2);
    ...
    coap_log(LIBCOAP_LOG_WARNING, "coap_pdu_encode_header: unsupported protocol\n");
  }
  return pdu->hdr_size;
}
