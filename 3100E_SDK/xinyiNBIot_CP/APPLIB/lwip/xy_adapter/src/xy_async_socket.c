...
static int send_msg_2_skt_thd(int msg_id, void *buff,int len)
{
	...
	if(skt_thd_msg_q == NULL)
		skt_thd_msg_q = osMessageQueueNew(12, sizeof(void *), NULL);
	if(skt_thd_h == NULL)
	{
		thread_attr.name	   = "skt_thd";
		thread_attr.priority   = osPriorityNormal1;
		thread_attr.stack_size = osStackShared;
		skt_thd_h = osThreadNew((osThreadFunc_t)(skt_proc_thread), NULL, &thread_attr);
	}
	UNLOCK_XY_SOCKET();
...
{
	xy_socket_mutux = osMutexNew(NULL);

}

err_t lwip_tcp_event(void *arg, struct tcp_pcb *pcb, enum lwip_event event_id, struct pbuf *p, u16_t size, err_t err)
{
#if VER_BC95
	xy_async_socket_api_event(arg, event_id, p, size, err);
#endif
	return ERR_OK;
}
