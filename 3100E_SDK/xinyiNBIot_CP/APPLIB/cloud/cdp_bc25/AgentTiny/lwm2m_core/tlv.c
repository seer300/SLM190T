                              ...
                              uint8_t *data_buffer)
{
    ...
    {
        length = 8;
        xy_utils_copyValue(data_buffer, &data, 8);
    }
    else
    {
        float value;
        ...
        length = 4;
        value = (float)data;
        xy_utils_copyValue(data_buffer, &value, 4);
    }
                            ...
                            uint8_t *data_buffer)
{
    ...
    else if (data >= INT32_MIN && data <= INT32_MAX)
    {
        ...
        value = data;
        length = 4;
        xy_utils_copyValue(data_buffer, &value, length);
    }
    else if (data >= INT64_MIN && data <= INT64_MAX)
    {
        length = 8;
        xy_utils_copyValue(data_buffer, &data, length);
    }

    return length;
}

static uint8_t prv_getHeaderType(xy_lwm2m_data_type_t type)
{
    switch (type)
    {
...
static xy_lwm2m_data_type_t prv_getDataType(uint8_t type)
{
    switch (type)
    {
...
int xy_tlv_parse(uint8_t *buffer,
              size_t bufferLen,
              lwm2m_data_t **dataP)
{
    xy_lwm2m_data_type_t type;
    ...
    *dataP = NULL;
    ...
    while (0 != (result = lwm2m_decode_TLV((uint8_t *)buffer + index, bufferLen - index, &type, &id, &dataIndex, &dataLen)))
    {
        ...
        *dataP = newTlvP;
        ...
        {
            (*dataP)[size].value.asChildren.count = xy_tlv_parse(buffer + index + dataIndex,
                                                    dataLen,
...
int xy_tlv_serialize(bool isResourceInstance,
                  int size,
                  lwm2m_data_t *dataP,
                  uint8_t **bufferP)
{
    ...
    *bufferP = (uint8_t *)lwm2m_malloc(length);
    ...
    for (i = 0 ; i < size && length != 0 ; i++)
    {
        ...
        case LWM2M_TYPE_OBJECT_INSTANCE:
        {
            ...
            res = xy_tlv_serialize(isInstance, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, &tmpBuffer);
            if (res < 0)
            {
                length = -1;
            }
