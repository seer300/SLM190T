                             ...
                             lwm2m_data_t *targetP)
{
    ...
    case _TYPE_FLOAT:
    {
        ...
        if (i == recordP->valueLen)
        {
            ...
            if ( 1 != xy_utils_textToInt(recordP->value,
                                      recordP->valueLen,
                                      &value))
            {
                return false;
        ...
        else
        {
            ...
            if ( 1 != xy_utils_textToFloat(recordP->value,
                                        recordP->valueLen,
                                        &value))
            {
                return false;
...
static xy_uri_depth_t prv_decreaseLevel(xy_uri_depth_t level)
{
    switch(level)
    {
    case XY_URI_DEPTH_OBJECT:
        return XY_URI_DEPTH_OBJECT_INSTANCE;
    case XY_URI_DEPTH_OBJECT_INSTANCE:
        return XY_URI_DEPTH_RESOURCE;
    case XY_URI_DEPTH_RESOURCE:
        return XY_URI_DEPTH_RESOURCE_INSTANCE;
    case XY_URI_DEPTH_RESOURCE_INSTANCE:
        return XY_URI_DEPTH_RESOURCE_INSTANCE;
    default:
        return XY_URI_DEPTH_RESOURCE;
    }
                             ...
                             lwm2m_data_t **dataP)
{
    ...
    else
    {
        ...
        *dataP = lwm2m_data_new(1);
        ...
        rootLevel = XY_URI_DEPTH_OBJECT_INSTANCE;
        parentP = *dataP;
        if (LWM2M_URI_IS_SET_INSTANCE(uriP))
        {
            ...
            rootLevel = XY_URI_DEPTH_RESOURCE;
            if (LWM2M_URI_IS_SET_RESOURCE(uriP))
            {
                ...
                rootLevel = XY_URI_DEPTH_RESOURCE_INSTANCE;
            }
        }
    ...
    for (index = 0 ; index < count ; index++)
    {
        ...
        if (targetP == NULL)
        {
            ...
            targetP->type = xy_utils_depthToDatatype(rootLevel);
        }
        if (recordArray[index].ids[1] != LWM2M_MAX_ID)
        {
            lwm2m_data_t *parentP;
            xy_uri_depth_t level;
            ...
            for (i = 1 ; i <= resSegmentIndex ; i++)
            {
                ...
                if (targetP == NULL)
                {
                    ...
                    targetP->type = xy_utils_depthToDatatype(level);
                }
                              ...
                              size_t bufferLen)
{
    ...
    case LWM2M_TYPE_INTEGER:
    {
        ...
        res = xy_utils_intToText(value, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
    ...
    case LWM2M_TYPE_FLOAT:
    {
        ...
        res = xy_utils_floatToText(value, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
    ...
    case LWM2M_TYPE_OPAQUE:
        ...
        res = xy_utils_base64Encode(tlvP->value.asBuffer.buffer, tlvP->value.asBuffer.length, buffer + head, bufferLen - head);
        if (res == 0) return -1;
        head += res;
                      ...
                      size_t bufferLen)
{
    ...
    case LWM2M_TYPE_MULTIPLE_RESOURCE:
    {
        ...
        res = xy_utils_intToText(tlvP->id, uriStr + uriLen, URI_MAX_STRING_LEN - uriLen);
        if (res <= 0) return -1;
        uriLen += res;
        uriStr[uriLen] = '/';
        uriLen++;
    ...
    default:
        ...
        res = xy_utils_intToText(tlvP->id, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
                   ...
                   uint8_t **bufferP)
{
    ...
    baseUriLen = xy_uri_toString(uriP, baseUriStr, URI_MAX_STRING_LEN, &rootLevel);
                ...
                || targetP->type == LWM2M_TYPE_MULTIPLE_RESOURCE))
    {
        ...
        res = xy_utils_intToText(targetP->id, baseUriStr + baseUriLen, URI_MAX_STRING_LEN - baseUriLen);
        if (res <= 0) return 0;
        baseUriLen += res;
