...
//待写入文件系统的数据总数 包括缓存总数和统计信息
#define CDP_TOTALBUFF_MAX_LEN  CDP_DOWNSTREAM_MAX_LEN + sizeof(cdp_session_info_t) + 11 * sizeof(int) 
...
void cdp_bak_downstream()
{
	...
	cdp_session_info_t* session_info = NULL;
	
	if(downstream_info == NULL)
		return;

	data_buf = xy_malloc(CDP_TOTALBUFF_MAX_LEN);
	//如果接收数据为0，再判断原文件系统里面是否有值，如果有值，覆盖写:
	if(cloud_read_file(CDP_SESSION_NVM_FILE_NAME, (void*)data_buf, CDP_TOTALBUFF_MAX_LEN) == XY_ERR)
	{
		//send_debug_by_at_uart("+DBGINFO:[cdp_bak_downstream] read downstream fail\r\n");
		xy_free(data_buf);
		return;
	}

	session_info = (cdp_session_info_t *)data_buf;
	memcpy(&received_num, data_buf + sizeof(cdp_session_info_t) + sizeof(int), sizeof(int));
	if((received_num <= 0 || received_num == 0xFFFFFFFF) && downstream_info->received_num == 0) //如果文件系统里面的值是0，或者没写过，然后再判断g_send_status
	{
		//send_debug_by_at_uart("+DBGINFO:[cdp_bak_downstream] received_num is 0\r\n");	
		if(g_send_status == session_info->cdp_con_send_status) //当文件系统里存的状态值与当前值不一样，则重新赋值；
		{
			xy_free(data_buf);
			return;
		}
	...
	g_cdp_session_info->cdp_con_send_status = g_send_status;//有下行缓存的时候，必须要写文件系统，所以这里直接赋值；
	memcpy(data_buf, g_cdp_session_info, sizeof(cdp_session_info_t));
	off_set += sizeof(cdp_session_info_t);
	
	memcpy(data_buf + off_set, &downstream_info->buffered_num, 3*sizeof(int));
	...
	cloud_save_file(CDP_SESSION_NVM_FILE_NAME,(void*)data_buf,CDP_TOTALBUFF_MAX_LEN);
	xy_free(data_buf);
}
...
void cdp_resume_downstream()
{
	...
	read_buf = xy_malloc(CDP_TOTALBUFF_MAX_LEN);
	memset(read_buf, 0x00, CDP_TOTALBUFF_MAX_LEN);

	if(cloud_read_file(CDP_SESSION_NVM_FILE_NAME,(void*)read_buf,CDP_TOTALBUFF_MAX_LEN)==XY_ERR)
	{
	...
	g_send_status = ((cdp_session_info_t *)read_buf)->cdp_con_send_status;
	memcpy(&downstream_info->buffered_num, read_buf + sizeof(cdp_session_info_t), 3*sizeof(int));
	offset_len += sizeof(cdp_session_info_t) + 3*sizeof(int);
	
	//从FLASH读取缓存数据挂链表
	for(i = 0; i < downstream_info->buffered_num; i++)
	{
		node = (buffer_list_t*)xy_malloc(sizeof(buffer_list_t));
        memset(node, 0x00, sizeof(buffer_list_t));
		
		memcpy(&node->data_len, read_buf + offset_len,sizeof(int));
		offset_len += sizeof(int);
        node->data = (char*)xy_malloc(node->data_len);
        memcpy(node->data,read_buf+offset_len,node->data_len);
        offset_len += node->data_len;
...
int cdp_delete_task()
{
	if(is_cdp_running())
	{
		...
		if(cdp_task_delete_sem == NULL)
			cdp_task_delete_sem = osSemaphoreNew(0xFFFF, 0, NULL);

	    osSemaphoreAcquire(cdp_task_delete_sem, 10*1000);

		//若等待信号量超时，且cdp还没退出，则断言
		if(is_cdp_running())
		{
			xy_assert(0);
		}

        osSemaphoreDelete(cdp_task_delete_sem);
        cdp_task_delete_sem = NULL;
