...
int at_NCDPOPEN_req(char *at_buf, char **prsp_cmd)
{
	if(g_req_type == AT_CMD_REQ)
	{
		...
		if (at_parse_param("%s,%d[0-65535],%s", at_buf, ip_addr_str, &port, psk_temp) != AT_OK || (strlen(ip_addr_str) <= 0)||(strlen(ip_addr_str) > 40) || (strlen(psk_temp) > 256)) 
		{
		...
		if(strlen(psk_temp) > 0 && strlen(psk_temp) <= 256)
		{
			...
			g_cdp_config_data->cdp_dtls_nat_type = 1;
...
int at_NCFG_req(char *at_buf, char **prsp_cmd)
{
	if(g_req_type == AT_CMD_REQ)
	{
		int mode = -1;
		int value = -1;
		if (at_parse_param("%d(0-1),%d[0-2592000]", at_buf, &mode, &value) != AT_OK || mode == 1 && value > 1) 
		{
			return ATERR_PARAM_INVALID;
		}
		...
		if(value == -1) //查询
		{
			*prsp_cmd = xy_malloc(40);
			...
			{
				snprintf(*prsp_cmd, 40, "%d",g_cdp_config_data->is_need_recover);
			}
			return AT_END;
		}
		...
		{
			g_cdp_config_data->is_need_recover = value; //开启深睡恢复功能，0表示开启，1表示关闭
		}
...
//AT+NMUPDATE=0/1 g_binding_mode 0:U,1:UQ
int at_NMUPDATE_req(char *at_buf, char **prsp_cmd)
{
    int ret = -1;
    int bind_mode = -1;
    if(g_req_type != AT_CMD_REQ)
	...
	if (at_parse_param("%d(0-1)", at_buf, &bind_mode) != AT_OK) 
	{
		return ATERR_PARAM_INVALID;
	}

    cdp_update_proc_bc25(targetP, bind_mode);
