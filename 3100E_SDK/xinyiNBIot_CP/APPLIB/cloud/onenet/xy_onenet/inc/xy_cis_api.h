...
typedef enum {
    CIS_OK                     =  0,
    CIS_ERR                    = -1,         //normal err
    CIS_Err_Timeout            = -2,         // Operation not completed within the timeout period.
    CIS_Err_NoConnected        = -3,         // tcpip newwork unusable,such as  PDP not activate
    CIS_Err_Parameter          = -4,         // input parameter error.
    CIS_Err_NoMemory           = -5,         // System is out of memory: it was impossible to allocate or reserve memory for the operation.
    CIS_Err_NotAllowed         = -6,         // api not allowed,such as SIM card not insert,and do something about 3GPP operation.
    CIS_Err_Reserved           = 0x7FFFFFFF
} cis_result_t;
