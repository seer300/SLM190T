                ...
                uint8_t raiflag)
{
    ...
    { /* a session exists, check if it is in state connected */
        ...
        {
            return dtls_send( ctx, peer, DTLS_CT_APPLICATION_DATA, buf, len, raiflag );
        }
    }
}
                                             ...
                                             uint8                      *buf )
{
    ...
    {
        cis_memset( buf, 0,sizeof(uint16)+sizeof(uint48) );
        buf += sizeof(uint16)+sizeof(uint48);
    }

    cis_memset( buf,0, sizeof(uint16) );
...
static int calculate_key_block( dtls_context_t              *ctx,
                                ...
                                dtls_peer_type               role )
	{
	  ...
	  /* Temporarily use the key_block storage space for the pre master secret. */
		...
		cis_memset(psk, 0, DTLS_PSK_MAX_KEY_LEN);
		if (pre_master_len < 0) {
	  ...
	  cis_memmove(handshake->tmp.master_secret, master_secret, DTLS_MASTER_SECRET_LENGTH);
...
{
    cis_memmove( hs_hash, &peer->handshake_params->hs_state.hs_hash,
                           ...
                           size_t          data_length )
{
    ...
    */
    ...
    cis_memmove( b.statebuf, &peer->handshake_params->hs_state.hs_hash, DTLS_HASH_CTX_SIZE );

    digest_length = finalize_hs_hash( peer, buf );
    /* clear_hash(); */

    /* restore hash status */
    cis_memmove( &peer->handshake_params->hs_state.hs_hash, b.statebuf, DTLS_HASH_CTX_SIZE );
                                ...
                                size_t                     *rlen )
{
    ...
    if ( !security || security->cipher == TLS_NULL_WITH_NULL_NULL )
    {
        ...
        for ( i = 0; i < data_array_len; i++ )
        {
            ...
            cis_memmove( p, data_array[i], data_len_array[i] );
            p += data_len_array[i];
...
#define A_DATA_LEN 13
        ...
        */
        ...
        cis_memmove( p, &DTLS_RECORD_HEADER( sendbuf )->epoch, 8 );
        p += 8;
        res = 8;
        ...
        cis_memset( nonce,0, DTLS_CCM_BLOCKSIZE );
        cis_memmove( nonce, dtls_kb_local_iv( security, peer->role ),
                dtls_kb_iv_size( security, peer->role ) );
        cis_memmove( nonce + dtls_kb_iv_size( security, peer->role ), start, 8 ); /* epoch + seq_num */
        ...
        */
        cis_memmove( A_DATA, &DTLS_RECORD_HEADER( sendbuf )->epoch, 8 ); /* epoch and seq_num */
        cis_memmove( A_DATA + 8, &DTLS_RECORD_HEADER( sendbuf )->content_type, 3 ); /* type and version */
                                         ...
                                         int              add_hash )
{
    ...
    return dtls_send_multi( ctx, peer, security, session, DTLS_CT_HANDSHAKE,
                            data_array, data_len_array, i, RAI_NULL );
}
                            ...
                            uint8_t raiflag)
{
    ...
    */
         ...
         type == DTLS_CT_CHANGE_CIPHER_SPEC )
    {
        ...
        if ( n )
        {
            ...
            for ( i = 0; i < buf_array_len; i++ )
            {
                cis_memmove( n->data + n->length, buf_array[i], buf_len_array[i] );
    ...
    */
    res = CALL( ctx, cis_dtls_write, session, sendbuf, len, raiflag );
                                   ...
                                   dtls_alert_t       description )
{
    ...
    dtls_send( ctx, peer, DTLS_CT_ALERT, msg, sizeof(msg) , RAI_NULL);
    return 0;
}
                                 ...
                                 dtls_peer_t    *peer )
{
    ...
    return dtls_send( ctx, peer, DTLS_CT_CHANGE_CIPHER_SPEC, buf, 1 , RAI_NULL);
}
                                          ...
                                          dtls_peer_t    *peer )
	{
	  ...
	  case TLS_PSK_WITH_AES_128_CCM_8: {
		...
		if (len + sizeof(uint16) > DTLS_CKXEC_LENGTH) {
		  cis_memset(&handshake->keyx.psk, 0, sizeof(dtls_handshake_parameters_psk_t));
		  dtls_warn("the psk identity is too long\n");
		  return dtls_alert_fatal_create(DTLS_ALERT_INTERNAL_ERROR);
		}
		handshake->keyx.psk.id_length = (unsigned int)len;
		cis_memmove(handshake->keyx.psk.identity, p + sizeof(uint16), len);
	
		dtls_int_to_uint16(p, handshake->keyx.psk.id_length);
		p += sizeof(uint16);
	
		cis_memmove(p, handshake->keyx.psk.identity, handshake->keyx.psk.id_length);
                                   ...
                                   size_t          cookie_length )
{
  ...
  cis_memcpy(p, handshake->tmp.random.client, DTLS_RANDOM_LENGTH);
  ...
  p += sizeof(uint8);
  if (cookie_length != 0) {
    cis_memcpy(p, cookie, cookie_length);
    p += cookie_length;
  }
                               ...
                               size_t          data_length )
{
    ...
    */
    ...
    cis_memmove( handshake->tmp.random.server, data, DTLS_RANDOM_LENGTH );
    /* skip server random */
			      ...
			      uint8 *data, size_t data_length)
{
  ...
  xy_assert(is_tls_psk_with_aes_128_ccm_8(config->cipher));
  ...
  cis_memcpy(config->keyx.psk.identity, data, len);
  return 0;
}
#endif /* DTLS_PSK */
#if CIS_ENABLE_ECC
			  ...
			  uint8 *data, size_t data_length)
{
  ...
  xy_assert(is_tls_ecdhe_ecdsa_with_aes_128_ccm_8(peer->handshake_params->cipher));
                                   ...
                                   size_t          data_length )
{
    ...
    */
		  ...
		  if (handshake->do_client_auth) {
			...
			res = dtls_send_certificate_ecdsa(ctx, peer);
...
#define A_DATA_LEN 13
        ...
        cis_memset( nonce,0, DTLS_CCM_BLOCKSIZE );
        cis_memmove( nonce, dtls_kb_remote_iv( security, peer->role ),
                dtls_kb_iv_size( security, peer->role ) );

        /* read epoch and seq_num from message */
        cis_memmove( nonce + dtls_kb_iv_size( security, peer->role ), *cleartext, 8 );
        *cleartext += 8;
        clen -= 8;
         ...
         *                   TLSCompressed.version + TLSCompressed.length;
        */
        cis_memmove( A_DATA, &DTLS_RECORD_HEADER( packet )->epoch, 8 ); /* epoch and seq_num */
        cis_memmove( A_DATA + 8, &DTLS_RECORD_HEADER( packet )->content_type, 3 ); /* type and version */
                             ...
                             size_t               data_length )
{
    ...
    else if ( dtls_uint16_to_int( hs_header->message_seq ) > peer->handshake_params->hs_state.mseq_r )
    {
        ...
        cis_memmove( n->data, data, data_length );
        ...
        * record. */
        ...
        switch ( msg[0] )
        {
            ...
            * epoch, Finish has epoch + 1. */
            ...
            dtls_stop_retransmission( ctx, peer );
            CALL( ctx, dtls_read, peer->session, data, data_length );
            break;
            default:
                       ...
                       void           *app_data )
{
    ...
    cis_memset( ctx, 0,sizeof(dtls_context_t) );
    ...
    ctx->cookie_secret_age = cissys_tick();
    return 0;
}
                             ...
                             uint8_t raiflag)
{
    ...
    if ( node->retransmit_cnt < DTLS_DEFAULT_MAX_RETRANSMIT )
    {
        ...
        (void)CALL( context, cis_dtls_write, node->peer->session, sendbuf, len, raiflag );
        return;
    }
    ...
    dtls_stop_retransmission(context, node->peer);
...
void dtls_check_retransmit( dtls_context_t *context)
{
    ...
    while ( node && node->t <= now )
    {
        ...
        dtls_retransmit( context, node , RAI_NULL);
...
dtls_send_certificate_ecdsa(dtls_context_t *ctx, dtls_peer_t *peer)
{
  ...
  dtls_int_to_uint24(p, targetP->dev_cert.raw.len+3);	
  p += sizeof(uint24);

  dtls_int_to_uint24(p, targetP->dev_cert.raw.len);	
  p += sizeof(uint24);
  
  cis_memcpy(p,targetP->dev_cert.raw.p,targetP->dev_cert.raw.len)
				...
				uint8 *data, size_t data_length)
{
  ...
  xy_assert(is_tls_ecdhe_ecdsa_with_aes_128_ccm_8(config->cipher));
  ...
  cis_memcpy(config->keyx.ecdsa.other_eph_pub_x, data, sizeof(config->keyx.ecdsa.other_eph_pub_y));
  data += sizeof(config->keyx.ecdsa.other_eph_pub_y);
  data_length -= sizeof(config->keyx.ecdsa.other_eph_pub_y);

  cis_memcpy(config->keyx.ecdsa.other_eph_pub_y, data, sizeof(config->keyx.ecdsa.other_eph_pub_y));
  data += sizeof(config->keyx.ecdsa.other_eph_pub_y);
  data_length -= sizeof(config->keyx.ecdsa.other_eph_pub_y);
