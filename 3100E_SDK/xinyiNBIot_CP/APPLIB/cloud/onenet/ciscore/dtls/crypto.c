...
static dtls_cipher_context_t *cipher_context = NULL;

static inline dtls_cipher_context_t *dtls_cipher_context_get( void )
{
    if(cipher_context == NULL)
    {
        cipher_context = xy_malloc(sizeof(dtls_cipher_context_t));
        memset(cipher_context, 0, sizeof(dtls_cipher_context_t));
    }
