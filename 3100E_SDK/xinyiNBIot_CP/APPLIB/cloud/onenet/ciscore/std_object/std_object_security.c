	...
	do {
		...
		case RES_SECURITY_URI_ID:
			...
			if (targetP->host != NULL)
			{
				...
				if(contextP->cloud_platform == CLOUD_PLATFORM_ANDLINK || (contextP->cloud_platform == CLOUD_PLATFORM_COMMON && contextP->platform_common_type == CLOUD_PLATFORM_COMMON_ANDLINK))
				{
                    utils_stringCopy(targetP->host, dataArray[i].asBuffer.length - 7 - URI_TAILER_HOST_LEN-1, (char*)dataArray[i].asBuffer.buffer + 7);//(URI_TAILER_HOST_LEN+1) : // ":15683"
                    cis_memset(contextP->serverPort, 0, utils_strlen(contextP->serverPort) + 1);
                    cis_memcpy(contextP->serverPort, (char*)dataArray[i].asBuffer.buffer + 7+strlen(targetP->host)+1,URI_TAILER_HOST_LEN);
				}
				else
				{
#if CIS_ENABLE_DTLS
...
#if CIS_ENABLE_DTLS
#if CIS_ENABLE_DM
	if (((contextP->cloud_platform == CLOUD_PLATFORM_ONENET && TRUE == isBootstrap && TRUE != contextP->isDM) ||
		 (contextP->cloud_platform == CLOUD_PLATFORM_COMMON) || (contextP->cloud_platform == CLOUD_PLATFORM_ANDLINK)) &&
		TRUE == contextP->isDTLS)
#else
	if (((contextP->cloud_platform == CLOUD_PLATFORM_ONENET && TRUE == isBootstrap) || (contextP->cloud_platform == CLOUD_PLATFORM_COMMON) || (contextP->cloud_platform == CLOUD_PLATFORM_ANDLINK)) && TRUE == contextP->isDTLS)
#endif
	{
#if CIS_ENABLE_PSK
		//read psk file and get the encrypt
		targetP->identify = (char*)cis_malloc(2 * NBSYS_IMEI_MAXLENGTH + 2);
		if (targetP->identify == NULL)
		{
		...
		cis_memset(targetP->identify, 0, 2 * NBSYS_IMEI_MAXLENGTH + 2);
		if((Get_Boot_Reason() != POWER_ON) && (g_onenet_session_info != NULL) && (strlen(g_onenet_session_info->endpointname) != 0))
	    {
	        memcpy(targetP->identify, g_onenet_session_info->endpointname, strlen(g_onenet_session_info->endpointname));
	    }
		else{
			xy_get_IMEI(targetP->identify, NBSYS_IMEI_MAXLENGTH);
		}	
...
void reset_bootstrap_instance(void* context)
{
	...
	while (securityInstance != NULL)
	{
		...
		if(!deleteInst->isBootstrap)
		{
			if (NULL != deleteInst->host)
			{
				cis_memset(deleteInst->host, 0, utils_strlen(deleteInst->host));
			}
#if CIS_ENABLE_DTLS
			if (NULL != deleteInst->identify)
			{
				cis_memset(deleteInst->identify, 0, utils_strlen(deleteInst->identify));
			}
			if (NULL != deleteInst->secret)
			{
				cis_memset(deleteInst->secret, 0, utils_strlen(deleteInst->secret));
			}
			if (NULL != deleteInst->public_key)
			{
				cis_memset(deleteInst->public_key, 0, utils_strlen(deleteInst->public_key));
			}
#endif
		}
