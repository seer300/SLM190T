...
extern cis_coapret_t std_device_execute(st_context_t *contextP, uint16_t instanceId, uint16_t resourceId, uint8_t *buffer, int length, st_object_t *objectP);
...
#if CIS_ENABLE_DM
extern cis_coapret_t std_dm_read(st_context_t * contextP, cis_iid_t instanceId, int * numDataP, st_data_t ** dataArrayP, st_object_t * objectP);
#endif
#if CIS_ENABLE_UPDATE
...
#if CIS_ENABLE_UPDATE
	{CIS_DEVICE_OBJECT_ID, std_device_read, std_device_write, std_device_execute, std_device_discover},
...
#endif//CIS_ENABLE_MONITER
#if CIS_ENABLE_DM
	{ LWM2M_DM_UE_OBJECT,std_dm_read,NULL,NULL,NULL },
#endif
};
