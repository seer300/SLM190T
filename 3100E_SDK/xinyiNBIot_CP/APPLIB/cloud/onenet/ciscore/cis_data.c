                           ...
                           bool * valueP)
{
    ...
    case DATA_TYPE_STRING:
    {
            ...
            if (dataP->asBuffer.length != 1) return 0;
...
int data_parse(st_uri_t * uriP,
                     ...
                     st_data_t ** dataP)
{
    LOGD("data_parse format: %s, bufferLen: %d", CIS_STR_MEDIA_TYPE(format), bufferLen);
                            ...
                            uint8_t ** bufferP)
{
    ...
    LOGD("Serialize final format: %s", CIS_STR_MEDIA_TYPE(*formatP));
