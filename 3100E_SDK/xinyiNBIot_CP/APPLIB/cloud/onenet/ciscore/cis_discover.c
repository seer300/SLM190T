                        ...
                        uint8_t ** bufferP )
{
    ...
    if(bufferLink == NULL)
    {
        LOGE( "discover serialize malloc failed.");
        return -1;
    }

    LOGD( "discover_serialize size: %d", size );
    baseUriLen = uri_toString( uriP, baseUriStr, URI_MAX_STRING_LEN, NULL );
    if ( baseUriLen < 0 ) goto TAG_FAILED;
    baseUriLen -= 1;
    ...
    if ( head > 0 )
    {
        ...
        *bufferP = (uint8_t *)cis_malloc( head );
        if ( *bufferP == NULL ) goto TAG_FAILED;
