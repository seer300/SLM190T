                         ...
                         _record_t * recordP)
{
    ...
    do
    {
        ...
        case 2:
        {
            ...
            case 'b':
                if (recordP->type != _TYPE_UNSET) return -1;
                if (0 == ctlw_lwm2m_strncmp(JSON_TRUE_STRING, (char *)buffer + index + valueStart, valueLen))
                {
                    recordP->type = _TYPE_TRUE;
                }
                else if (0 == ctlw_lwm2m_strncmp(JSON_FALSE_STRING, (char *)buffer + index + valueStart, valueLen))
                {
                             ...
                             lwm2m_data_t * targetP)
{
    ...
    {
    case _TYPE_FALSE:
        ctlw_lwm2m_data_encode_bool(false, targetP);
        break;
    case _TYPE_TRUE:
        ctlw_lwm2m_data_encode_bool(true, targetP);
        break;
    case _TYPE_FLOAT:
    {
        size_t i;
        ...
        if (i == recordP->valueLen)
        {
            ...
            if ( 1 != ctlw_utils_textToInt(recordP->value,
                                      recordP->valueLen,
                                      &value))
            {
                return false;
            ...
            ctlw_lwm2m_data_encode_int(value, targetP);
        }
        else
        {
            double value;

            if ( 1 != ctlw_utils_textToFloat(recordP->value,
                                        recordP->valueLen,
                                        &value))
            {
                return false;
            ...
            ctlw_lwm2m_data_encode_float(value, targetP);
        }
    }
    break;

    case _TYPE_STRING:
        ctlw_lwm2m_data_encode_opaque(recordP->value, recordP->valueLen, targetP);
...
static lwm2m_data_t * prv_extendData(lwm2m_data_t * parentP)
{
    ...
    newP = ctlw_lwm2m_data_new(parentP->value.asChildren.count + 1);
    ...
    if (parentP->value.asChildren.array != NULL)
    {
        ...
        ctlw_lwm2m_free(parentP->value.asChildren.array);     // do not use ctlw_lwm2m_data_free() to keep pointed values
    }
                             ...
                             lwm2m_data_t ** dataP)
{
    ...
    {
        size = count;
        *dataP = ctlw_lwm2m_data_new(count);
    ...
    else
    {
        ...
        *dataP = ctlw_lwm2m_data_new(1);
        ...
        {
            parentP->value.asChildren.count = 1;
            parentP->value.asChildren.array = ctlw_lwm2m_data_new(1);
        ...
        parentP->value.asChildren.array = ctlw_lwm2m_data_new(count);
        if (NULL == parentP->value.asChildren.array) goto error;
    ...
    for (index = 0 ; index < count ; index++)
    {
        ...
        if (targetP == NULL)
        {
            ...
            targetP->type = ctlw_utils_depthToDatatype(rootLevel);
        }
        if (recordArray[index].ids[1] != LWM2M_MAX_ID)
        {
            ...
            for (i = 1 ; i <= resSegmentIndex ; i++)
            {
                ...
                if (targetP == NULL)
                {
                    ...
                    targetP->type = ctlw_utils_depthToDatatype(level);
                }
                         ...
                         lwm2m_data_t ** resultP)
{
    ...
    *resultP = ctlw_lwm2m_data_new(realSize);
...
int ctlw_json_parse(lwm2m_uri_t * uriP,
               uint8_t * buffer,
               size_t bufferLen,
               lwm2m_data_t ** dataP)
{
    ...
    *dataP = NULL;
    ...
    do
    {
        ...
        case 'e':
        {
            ...
            recordArray = (_record_t*)ctlw_lwm2m_malloc(count * sizeof(_record_t));
    ...
    if (eFound == true)
    {
        ...
        else
        {
            ...
            {
                res = ctlw_lwm2m_stringToUri((char *)buffer + bnStart, bnLen, &baseURI);
                    ...
                    for (i = 0 ; i < size && resP == NULL; i++)
                    {
                        ...
                        if (targetP->id == uriP->resourceId)
                        {
                            ...
                            else
                            {
                                ...
                                ctlw_lwm2m_data_free(count, parsedP);
                                parsedP = NULL;
                            }
                        }
                    }
                              ...
                              size_t bufferLen)
{
    ...
    case LWM2M_TYPE_FLOAT:
    {
        ...
        if (0 == ctlw_lwm2m_data_decode_float(tlvP, &value)) return -1;

        if (bufferLen < JSON_ITEM_NUM_SIZE) return -1;
        memcpy(buffer, JSON_ITEM_NUM, JSON_ITEM_NUM_SIZE);
        head = JSON_ITEM_NUM_SIZE;

        res = ctlw_utils_floatToText(value, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
    ...
    case LWM2M_TYPE_BOOLEAN:
    {
        ...
        if (0 == ctlw_lwm2m_data_decode_bool(tlvP, &value)) return -1;
    ...
    case LWM2M_TYPE_OPAQUE:
        ...
        res = ctlw_utils_base64Encode(tlvP->value.asBuffer.buffer, tlvP->value.asBuffer.length, buffer+head, bufferLen - head);
        if (res == 0) return -1;
        head += res;
                      ...
                      size_t bufferLen)
{
    ...
    case LWM2M_TYPE_MULTIPLE_RESOURCE:
    {
        ...
        res = ctlw_utils_intToText(tlvP->id, uriStr + uriLen, URI_MAX_STRING_LEN - uriLen);
        if (res <= 0) return -1;
        uriLen += res;
        uriStr[uriLen] = '/';
        uriLen++;
    ...
    default:
        ...
        res = ctlw_utils_intToText(tlvP->id, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
...
int ctlw_json_serialize(lwm2m_uri_t * uriP,
                   int size,
                   lwm2m_data_t * tlvP,
                   uint8_t ** bufferP)
{
    ...
    baseUriLen = ctlw_uri_toString(uriP, baseUriStr, URI_MAX_STRING_LEN, &rootLevel);
         ...
         || targetP->type == LWM2M_TYPE_MULTIPLE_RESOURCE))
    {
        ...
        res = ctlw_utils_intToText(targetP->id, baseUriStr + baseUriLen, URI_MAX_STRING_LEN - baseUriLen);
        if (res <= 0) return 0;
        baseUriLen += res;
    ...
    *bufferP = (uint8_t *)ctlw_lwm2m_malloc(head);
    if (*bufferP == NULL) 
		return 0;
	
    memcpy(*bufferP, bufferJSON, head);
	
    LOG_ARG("json buffer:%s\n",*bufferP);
