                               ...
                               ctlw_coap_packet_t * message)
{
    if (COAP_204_CHANGED == message->code)
    {
        ...
        bootstrapServer->registration = ctlw_lwm2m_gettime() + COAP_EXCHANGE_LIFETIME;
    }
    else
    {
                                     ...
                                     void * message)
{
    ...
    ctlw_coap_packet_t * coapMessage = (ctlw_coap_packet_t *)message;
                                 ...
                                 lwm2m_server_t * bootstrapServer)
{
    ...
    query_length = ctlw_utils_stringCopy(query, PRV_QUERY_BUFFER_LENGTH, QUERY_STARTER QUERY_NAME);
    if (query_length < 0)
    {
        bootstrapServer->status = STATE_BS_FAILING;
        return;
    }
    res = ctlw_utils_stringCopy(query + query_length, PRV_QUERY_BUFFER_LENGTH - query_length, context->endpointName);
    if (res < 0)
    {
        bootstrapServer->status = STATE_BS_FAILING;
        return;
    ...
    {
        bootstrapServer->sessionH = ctlw_lwm2m_connect_server(bootstrapServer->secObjInstID, context->userData);
    }
    ...
    if (bootstrapServer->sessionH != NULL)
    {
		...
		//----add----
        transaction = ctlw_transaction_new(bootstrapServer->sessionH, COAP_POST, NULL, NULL, ctlw_lwm2m_get_next_mid(context), 4, NULL);
        //-----------
		if (transaction == NULL)
        {
            bootstrapServer->status = STATE_BS_FAILING;
        ...
        ctlw_coap_set_header_uri_path(transaction->message, "/"URI_BOOTSTRAP_SEGMENT);
        ...
        if (ctlw_transaction_send(context, transaction) == 0)
        {
...
void ctlw_bootstrap_step(lwm2m_context_t * contextP,
...
uint8_t ctlw_bootstrap_handleFinish(lwm2m_context_t * context,
                               void * fromSessionH)
{
    lwm2m_server_t * bootstrapServer;

    LOG("Entering");
    bootstrapServer = ctlw_utils_findBootstrapServer(context, fromSessionH);
...
void ctlw_bootstrap_start(lwm2m_context_t * contextP)
{
    ...
    while (targetP != NULL)
    {
        ...
        if (targetP->sessionH == NULL)
        {
            targetP->sessionH = ctlw_lwm2m_connect_server(targetP->secObjInstID, contextP->userData);
        }
        targetP = targetP->next;
    }
}
...
static uint8_t prv_checkServerStatus(lwm2m_server_t * serverP)
{
    ...
    case STATE_BS_PENDING:
        serverP->registration = ctlw_lwm2m_gettime() + COAP_EXCHANGE_LIFETIME;
        break;
...
uint8_t ctlw_bootstrap_handleCommand(lwm2m_context_t * contextP,
                                lwm2m_uri_t * uriP,
                                lwm2m_server_t * serverP,
                                ctlw_coap_packet_t * message,
    ...
    case COAP_PUT:
        {
            ...
            {
                if (ctlw_object_isInstanceNew(contextP, uriP->objectId, uriP->instanceId))
                {
            ...
            else
            {
                ...
                {
                    size = ctlw_lwm2m_data_parse(uriP, message->payload, message->payload_len, format, &dataP);
                    if (size == 0)
                    {
                    ...
                    for (i = 0 ; i < size ; i++)
                    {
                        ...
                        {
                            if (ctlw_object_isInstanceNew(contextP, uriP->objectId, dataP[i].id))
                            {
                                result = ctlw_object_createInstance(contextP, uriP, &dataP[i]);
                                if (COAP_201_CREATED == result)
                                {
                                    result = COAP_204_CHANGED;
                                }
                            ...
                            {
                                result = ctlw_object_writeInstance(contextP, uriP, &dataP[i]);
                                if (uriP->objectId == LWM2M_SECURITY_OBJECT_ID
                                 && result == COAP_204_CHANGED)
                                {
...
uint8_t ctlw_bootstrap_handleDeleteAll(lwm2m_context_t * contextP,
                                  void * fromSessionH)
{
    lwm2m_server_t * serverP;
    uint8_t result;
        ...
        if (objectP->objID == LWM2M_SECURITY_OBJECT_ID)
        {
                ...
                && result == COAP_202_DELETED)
            {
                ...
                else
                {
                    ...
                    result = ctlw_object_delete(contextP, &uri);
                    instanceP = objectP->instanceList;
                }
            }
            if (result == COAP_202_DELETED)
...
uint8_t ctlw_bootstrap_handleRequest(lwm2m_context_t * contextP,
                                lwm2m_uri_t * uriP,
                                void * fromSessionH,
                                ctlw_coap_packet_t * message,
                                ctlw_coap_packet_t * response)
{
    ...
    if (ctlw_lwm2m_strncmp((char *)message->uri_query->data, QUERY_NAME, QUERY_NAME_LEN) != 0)
    {
    ...
    name = (char *)ctlw_lwm2m_malloc(message->uri_query->len - QUERY_NAME_LEN + 1);
    ...
    ctlw_lwm2m_free(name);

    return result;
}

void ctlw_lwm2m_set_bootstrap_callback(lwm2m_context_t * contextP,
...
int ctlw_lwm2m_bootstrap_delete(lwm2m_context_t * contextP,
                           void * sessionH,
                           lwm2m_uri_t * uriP)
{
    ...
    LOG_URI(uriP);
	//----modified----
    transaction = ctlw_transaction_new(sessionH, COAP_DELETE, NULL, uriP, ctlw_lwm2m_get_next_mid(contextP), 4, NULL);
	//--------------
    if (transaction == NULL) return COAP_500_INTERNAL_SERVER_ERROR;

    dataP = (bs_data_t *)ctlw_lwm2m_malloc(sizeof(bs_data_t));
    if (dataP == NULL)
    {
    ...
    return ctlw_transaction_send(contextP, transaction);
}

int ctlw_lwm2m_bootstrap_write(lwm2m_context_t * contextP,
                          void * sessionH,
                          lwm2m_uri_t * uriP,
                          lwm2m_media_type_t format,
                          uint8_t * buffer,
                          size_t length)
{
	...
	//----modified----
    transaction = ctlw_transaction_new(sessionH, COAP_PUT, NULL, uriP, ctlw_lwm2m_get_next_mid(contextP), 4, NULL);
	//-----------
    if (transaction == NULL) return COAP_500_INTERNAL_SERVER_ERROR;

    ctlw_coap_set_header_content_type(transaction->message, format);
    ctlw_coap_set_payload(transaction->message, buffer, length);
...
int ctlw_lwm2m_bootstrap_finish(lwm2m_context_t * contextP,
                           void * sessionH)
{
    lwm2m_transaction_t * transaction;
    bs_data_t * dataP;
    ...
    LOG("Entering");
	//----modified----
    transaction = ctlw_transaction_new(sessionH, COAP_POST, NULL, NULL, ctlw_lwm2m_get_next_mid(contextP), 4, NULL);
	//----------
    if (transaction == NULL) return COAP_500_INTERNAL_SERVER_ERROR;
