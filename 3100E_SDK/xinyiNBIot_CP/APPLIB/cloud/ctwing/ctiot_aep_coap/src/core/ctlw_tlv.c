                              ...
                              uint8_t * data_buffer)
{
    ...
    {
        length = 8;
        ctlw_utils_copyValue(data_buffer, &data, 8);
    }
    else
    {
        float value;
        ...
        length = 4;
        value = (float)data;
        ctlw_utils_copyValue(data_buffer, &value, 4);
    }
                            ...
                            uint8_t * data_buffer)
{
    ...
    else if (data >= INT32_MIN && data <= INT32_MAX)
    {
        ...
        value = data;
        length = 4;
        ctlw_utils_copyValue(data_buffer, &value, length);
    }
    else if (data >= INT64_MIN && data <= INT64_MAX)
    {
        length = 8;
        ctlw_utils_copyValue(data_buffer, &data, length);
    }
...
int ctlw_lwm2m_decode_TLV(const uint8_t * buffer,
                    size_t buffer_len,
...
int ctlw_tlv_parse(uint8_t * buffer,
              size_t bufferLen,
              lwm2m_data_t ** dataP)
{
    lwm2m_data_type_t type;
    ...
    *dataP = NULL;
    ...
    while (0 != (result = ctlw_lwm2m_decode_TLV((uint8_t*)buffer + index, bufferLen - index, &type, &id, &dataIndex, &dataLen)))
    {
        lwm2m_data_t * newTlvP;

        newTlvP = ctlw_lwm2m_data_new(size + 1);
        if (size >= 1)
        {
            if (newTlvP == NULL)
            {
        ...
        *dataP = newTlvP;
        ...
        {
            (*dataP)[size].value.asChildren.count = ctlw_tlv_parse(buffer + index + dataIndex,
                                                          dataLen,
                                                          &((*dataP)[size].value.asChildren.array));
            if ((*dataP)[size].value.asChildren.count == 0)
            {
                ctlw_lwm2m_data_free(size + 1, *dataP);
                return 0;
            }
        }
        else
        {
            ctlw_lwm2m_data_encode_opaque(buffer + index + dataIndex, dataLen, (*dataP) + size);
        }
        size++;
        index += result;
    }
...
int ctlw_tlv_serialize(bool isResourceInstance, 
                  int size,
                  lwm2m_data_t * dataP,
                  uint8_t ** bufferP)
{
    ...
    //printf("isResourceInstance: %s, size: %d", isResourceInstance?"true":"false", size);
    *bufferP = NULL;
    length = prv_getLength(size, dataP);
//    printf("tlv get len:%d\n",length);
    ...
    *bufferP = (uint8_t *)ctlw_lwm2m_malloc(length);
    ...
    if (*bufferP == NULL) 
    {
        LOG_ARG("tlv buffer malloc:null\n");
        return 0;
    }
    index = 0;
        ...
        case LWM2M_TYPE_OBJECT_INSTANCE:
            {
                ...
                res = ctlw_tlv_serialize(isInstance, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, &tmpBuffer);
                if (res < 0)
                {
                    length = -1;
                }
