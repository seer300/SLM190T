                           ...
                           location_data_t *locDataP)
{
    ...
    {
    case RES_M_LATITUDE:
        ctlw_lwm2m_data_encode_float(locDataP->latitude, dataP);
        break;
    case RES_M_LONGITUDE:
        ctlw_lwm2m_data_encode_float(locDataP->longitude, dataP);
        break;
    case RES_O_ALTITUDE:
        ctlw_lwm2m_data_encode_float(locDataP->altitude, dataP);
        break;
    case RES_O_RADIUS:
        ctlw_lwm2m_data_encode_float(locDataP->radius, dataP);
        break;
    case RES_O_VELOCITY:
        ctlw_lwm2m_data_encode_string((const char *)locDataP->velocity, dataP);
        break;
    case RES_M_TIMESTAMP:
        ctlw_lwm2m_data_encode_int(locDataP->timestamp, dataP);
        break;
    case RES_O_SPEED:
        ctlw_lwm2m_data_encode_float(locDataP->speed, dataP);
        break;
    default:
                                 ...
                                 ctlw_lwm2m_object_t *objectP)
{
    ...
    if (*numDataP == 0) // full object, readable resources!
    {
        ...
        *tlvArrayP = ctlw_lwm2m_data_new(*numDataP);
        if (*tlvArrayP == NULL)
...
void ctlw_display_location_object(ctlw_lwm2m_object_t *object)
{
#ifdef WITH_LOGS
    location_data_t *data = (location_data_t *)object->userData;
    ctiot_log_debug(LOG_COMMON_MODULE,LOG_OTHER_CLASS,"  /%u: Location object:\r\n", object->objID);
    if (NULL != data)
    {
        ctiot_log_debug(LOG_COMMON_MODULE,LOG_OTHER_CLASS,"    latitude: %.6f, longitude: %.6f, altitude: %.6f, radius: %.6f, timestamp: %lu, speed: %.6f\r\n",
...
/**
  ...
  */
void ctlw_location_setVelocity(ctlw_lwm2m_object_t *locationObj,
                          uint16_t bearing,
...
/**
  ...
  */
void ctlw_location_setLocationAtTime(ctlw_lwm2m_object_t *locationObj,
                                float latitude,
                                float longitude,
                                float altitude,
                                uint64_t timestamp)
...
{
    //-------------------------------------------------------------------- JH --
    ctlw_lwm2m_object_t *locationObj;

    locationObj = (ctlw_lwm2m_object_t *)ctlw_lwm2m_malloc(sizeof(ctlw_lwm2m_object_t));
    if (NULL != locationObj)
    {
        memset(locationObj, 0, sizeof(ctlw_lwm2m_object_t));
        ...
        locationObj->instanceList = (lwm2m_list_t *)ctlw_lwm2m_malloc(sizeof(lwm2m_list_t));
        ...
        locationObj->userData = ctlw_lwm2m_malloc(sizeof(location_data_t));
        ...
        if (NULL != locationObj->userData)
        {
            ...
            data->radius = 0.0;
            ctlw_location_setVelocity(locationObj, 0, 0, 255); // 255: speedUncertainty not supported!
            data->timestamp = (unsigned long)ctlw_lwm2m_gettime();
            data->speed = 0.0;
        }
        else
        {
...
{
    ctlw_lwm2m_list_free(object->instanceList);
