...
static uint8_t prv_set_tlv(lwm2m_data_t *dataP, acc_ctrl_oi_t *accCtrlOiP)
{
    ...
    case RES_M_OBJECT_ID:
        ctlw_lwm2m_data_encode_int(accCtrlOiP->objectId, dataP);
        return COAP_205_CONTENT;
        //break;
    case RES_M_OBJECT_INSTANCE_ID:
        ctlw_lwm2m_data_encode_int(accCtrlOiP->objectInstId, dataP);
        return COAP_205_CONTENT;
        //break;
    case RES_O_ACL:
    {
        ...
        {
            lwm2m_data_t *subTlvP = ctlw_lwm2m_data_new(ri);
            for (accCtrlRiP = accCtrlOiP->accCtrlValList, ri = 0;
                 accCtrlRiP != NULL;
                 accCtrlRiP = accCtrlRiP->next, ri++)
            {
                subTlvP[ri].id = accCtrlRiP->resInstId;
                ctlw_lwm2m_data_encode_int(accCtrlRiP->accCtrlValue, &subTlvP[ri]);
            }
            ctlw_lwm2m_data_encode_instances(subTlvP, 2, dataP);
            return COAP_205_CONTENT;
        }
    }
    //break;
    case RES_M_ACCESS_CONTROL_OWNER:
        ctlw_lwm2m_data_encode_int(accCtrlOiP->accCtrlOwner, dataP);
        return COAP_205_CONTENT;
        //break;
    default:
        return COAP_404_NOT_FOUND;
...
static uint8_t prv_read(uint16_t instanceId, int *numDataP,
                        lwm2m_data_t **dataArrayP, ctlw_lwm2m_object_t *objectP)
{
    uint8_t result;
    int ri, ni;
    ...
    acc_ctrl_oi_t *accCtrlOiP =
        (acc_ctrl_oi_t *)ctlw_lwm2m_list_find(objectP->instanceList, instanceId);
    if (accCtrlOiP == NULL)
    {
        return COAP_404_NOT_FOUND;
    }
                           ...
                           uint16_t acResId, uint16_t acValue)
{
    bool ret = false;
    acc_ctrl_ri_t *accCtrlRiP;
    accCtrlRiP = (acc_ctrl_ri_t *)ctlw_lwm2m_malloc(sizeof(acc_ctrl_ri_t));
    if (accCtrlRiP == NULL)
    {
        return ret;
    }
...
static uint8_t prv_write_resources(uint16_t instanceId, int numData,
                                   lwm2m_data_t *tlvArray, ctlw_lwm2m_object_t *objectP, bool doCreate)
{
    int i;
    uint8_t result;
    int64_t value;
    ...
    acc_ctrl_oi_t *accCtrlOiP = (acc_ctrl_oi_t *)
        ctlw_lwm2m_list_find(objectP->instanceList, instanceId);
    if (NULL == accCtrlOiP)
        return COAP_404_NOT_FOUND;
    ...
    do
    {
        ...
        case RES_M_OBJECT_ID:
            ...
            {
                if (1 != ctlw_lwm2m_data_decode_int(&tlvArray[i], &value))
                {
        ...
        case RES_O_ACL:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        if (1 != ctlw_lwm2m_data_decode_int(&subTlvArray[ri], &value))
                        {
                            result = COAP_400_BAD_REQUEST;
                            break;
                        }
...
static uint8_t prv_write(uint16_t instanceId, int numData,
                         lwm2m_data_t *tlvArray, ctlw_lwm2m_object_t *objectP)
{
    return prv_write_resources(instanceId, numData, tlvArray, objectP, false);
}

static uint8_t prv_delete(uint16_t id, ctlw_lwm2m_object_t *objectP)
{
    acc_ctrl_oi_t *targetP;

    objectP->instanceList = ctlw_lwm2m_list_remove(objectP->instanceList, id,
                          ...
                          lwm2m_data_t *tlvArray, ctlw_lwm2m_object_t *objectP)
{
    ...
    targetP = (acc_ctrl_oi_t *)ctlw_lwm2m_malloc(sizeof(acc_ctrl_oi_t));
    if (NULL == targetP)
        return COAP_500_INTERNAL_SERVER_ERROR;
...
{
    /*
     * The ctlw_acc_ctrl_create_object() function creates an empty object
     * and returns a pointer to the structure that represents it.
     */
    ctlw_lwm2m_object_t *accCtrlObj = NULL;

    accCtrlObj = (ctlw_lwm2m_object_t *)ctlw_lwm2m_malloc(sizeof(ctlw_lwm2m_object_t));

    if (NULL != accCtrlObj)
    {
        memset(accCtrlObj, 0, sizeof(ctlw_lwm2m_object_t));
        /*
...
void ctlw_ctlw_acl_ctrl_free_object(ctlw_lwm2m_object_t *objectP)
{
    ...
    while (accCtrlOiP != NULL)
    {
        ...
        ctlw_lwm2m_free(accCtrlOiT);
    }
    ctlw_lwm2m_free(objectP);
}

bool ctlw_ctlw_acc_ctrl_obj_add_inst(ctlw_lwm2m_object_t *accCtrlObjP, uint16_t instId,
                           uint16_t acObjectId, uint16_t acObjInstId, uint16_t acOwner)
{
    ...
    else
    {
        ...
        acc_ctrl_oi_t *accCtrlOiP;
        accCtrlOiP = (acc_ctrl_oi_t *)ctlw_lwm2m_malloc(sizeof(acc_ctrl_oi_t));
        if (NULL == accCtrlOiP)
        {
            return ret;
        }
                            ...
                            uint16_t acResId, uint16_t acValue)
{
    ...
    acc_ctrl_oi_t *accCtrlOiP = (acc_ctrl_oi_t *)
        ctlw_lwm2m_list_find(accCtrlObjP->instanceList, instId);
    if (NULL == accCtrlOiP)
        return ret;
