                             ...
                             security_instance_t *targetP)
{
    ...
    case LWM2M_SECURITY_URI_ID:
        ctlw_lwm2m_data_encode_string(targetP->uri, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_BOOTSTRAP_ID:
        ctlw_lwm2m_data_encode_bool(targetP->isBootstrap, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SECURITY_ID:
        ctlw_lwm2m_data_encode_int(targetP->securityMode, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_PUBLIC_KEY_ID:
        ctlw_lwm2m_data_encode_opaque((uint8_t *)targetP->publicIdentity, targetP->publicIdLen, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SERVER_PUBLIC_KEY_ID:
        ctlw_lwm2m_data_encode_opaque((uint8_t *)targetP->serverPublicKey, targetP->serverPublicKeyLen, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SECRET_KEY_ID:
        ctlw_lwm2m_data_encode_opaque((uint8_t *)targetP->secretKey, targetP->secretKeyLen, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SMS_SECURITY_ID:
        ctlw_lwm2m_data_encode_int(targetP->smsSecurityMode, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SMS_KEY_PARAM_ID:
        ctlw_lwm2m_data_encode_opaque((uint8_t *)targetP->smsParams, targetP->smsParamsLen, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_SMS_SECRET_KEY_ID:
        ctlw_lwm2m_data_encode_opaque((uint8_t *)targetP->smsSecret, targetP->smsSecretLen, dataP);
        return COAP_205_CONTENT;
    ...
    case LWM2M_SECURITY_SHORT_SERVER_ID:
        ctlw_lwm2m_data_encode_int(targetP->shortID, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_HOLD_OFF_ID:
        ctlw_lwm2m_data_encode_int(targetP->clientHoldOffTime, dataP);
        return COAP_205_CONTENT;

    case LWM2M_SECURITY_BOOTSTRAP_TIMEOUT_ID:
        ctlw_lwm2m_data_encode_int(targetP->bootstrapServerAccountTimeout, dataP);
        return COAP_205_CONTENT;
                                 ...
                                 ctlw_lwm2m_object_t *objectP)
{
    ...
    targetP = (security_instance_t *)ctlw_lwm2m_list_find(objectP->instanceList, instanceId);
    if (NULL == targetP)
        return COAP_404_NOT_FOUND;
                                  ...
                                  ctlw_lwm2m_object_t *objectP)
{
    ...
    do
    {
        ...
        case LWM2M_SECURITY_URI_ID:
            if (targetP->uri != NULL)
                ctlw_lwm2m_free(targetP->uri);
            targetP->uri = (char *)ctlw_lwm2m_malloc(dataArray[i].value.asBuffer.length + 1);
        ...
        case LWM2M_SECURITY_BOOTSTRAP_ID:
            if (1 == ctlw_lwm2m_data_decode_bool(dataArray + i, &(targetP->isBootstrap)))
            {
                result = COAP_204_CHANGED;
            }
            else
        ...
        case LWM2M_SECURITY_SECURITY_ID:
        {
            ...
            if (1 == ctlw_lwm2m_data_decode_int(dataArray + i, &value))
            {
                if (value >= 0 && value <= 3)
                {
        ...
        case LWM2M_SECURITY_PUBLIC_KEY_ID:
            if (targetP->publicIdentity != NULL)
                ctlw_lwm2m_free(targetP->publicIdentity);
            targetP->publicIdentity = (char *)ctlw_lwm2m_malloc(dataArray[i].value.asBuffer.length + 1);
        ...
        case LWM2M_SECURITY_SERVER_PUBLIC_KEY_ID:
            if (targetP->serverPublicKey != NULL)
                ctlw_lwm2m_free(targetP->serverPublicKey);
            targetP->serverPublicKey = (char *)ctlw_lwm2m_malloc(dataArray[i].value.asBuffer.length + 1);
        ...
        case LWM2M_SECURITY_SECRET_KEY_ID:
            if (targetP->secretKey != NULL)
                ctlw_lwm2m_free(targetP->secretKey);
            targetP->secretKey = (char *)ctlw_lwm2m_malloc(dataArray[i].value.asBuffer.length + 1);
                                   ...
                                   ctlw_lwm2m_object_t *objectP)
{
    ...
    targetP = (security_instance_t *)ctlw_lwm2m_malloc(sizeof(security_instance_t));
    if (NULL == targetP)
        return COAP_500_INTERNAL_SERVER_ERROR;
...
void ctlw_copy_security_object(ctlw_lwm2m_object_t *objectDest, ctlw_lwm2m_object_t *objectSrc)
{
    ...
    {
        security_instance_t *instanceDest = (security_instance_t *)ctlw_lwm2m_malloc(sizeof(security_instance_t));
        if (NULL == instanceDest)
        {
            return;
        }
        memcpy(instanceDest, instanceSrc, sizeof(security_instance_t));
        instanceDest->uri = (char *)ctlw_lwm2m_malloc(strlen(instanceSrc->uri) + 1);
        strcpy(instanceDest->uri, instanceSrc->uri);
        if (instanceSrc->securityMode == LWM2M_SECURITY_MODE_PRE_SHARED_KEY)
        {
            instanceDest->publicIdentity = ctlw_lwm2m_strdup(instanceSrc->publicIdentity);
            instanceDest->secretKey = ctlw_lwm2m_strdup(instanceSrc->secretKey);
        }
...
void ctlw_display_security_object(ctlw_lwm2m_object_t *object)
{
#ifdef WITH_LOGS
    ctiot_log_info(LOG_SEND_RECV_MODULE, LOG_OTHER_CLASS, "  /%u: Security object, instances:\r\n", object->objID);
    security_instance_t *instance = (security_instance_t *)object->instanceList;
    while (instance != NULL)
    {
        ctiot_log_info(LOG_SEND_RECV_MODULE, LOG_OTHER_CLASS, "    /%u/%u: instanceId: %u, uri: %s, isBootstrap: %s, shortId: %u, clientHoldOffTime: %u\r\n",
...
void ctlw_clean_security_object(ctlw_lwm2m_object_t *objectP)
{
    while (objectP->instanceList != NULL)
    {
        ...
        {
            ctlw_lwm2m_free(securityInstance->uri);
        }
        if (securityInstance->securityMode == LWM2M_SECURITY_MODE_PRE_SHARED_KEY)
        {
            ctlw_lwm2m_free(securityInstance->publicIdentity);
            ctlw_lwm2m_free(securityInstance->secretKey);
        }
        ctlw_lwm2m_free(securityInstance);
    }
}

ctlw_lwm2m_object_t *ctlw_get_security_object(int serverId,
                                    ...
                                    bool isBootstrap)
{
    ctlw_lwm2m_object_t *securityObj;

    securityObj = (ctlw_lwm2m_object_t *)ctlw_lwm2m_malloc(sizeof(ctlw_lwm2m_object_t));

    if (NULL != securityObj)
    {
        security_instance_t *targetP;

        memset(securityObj, 0, sizeof(ctlw_lwm2m_object_t));
        ...
        if (NULL == targetP)
        {
            ctlw_lwm2m_free(securityObj);
            return NULL;
        }

        memset(targetP, 0, sizeof(security_instance_t));
        targetP->instanceId = 0;
        targetP->uri = (char *)ctlw_lwm2m_malloc(strlen(serverUri) + 1);
        ...
        if (bsPskId != NULL || psk != NULL)
        {
            ...
            if (bsPskId)
            {
                targetP->publicIdentity = ctlw_lwm2m_strdup(bsPskId);
                targetP->publicIdLen = strlen(bsPskId);
            }
            if (pskLen > 0)
            {
                targetP->secretKey = (char *)ctlw_lwm2m_malloc(pskLen);
                if (targetP->secretKey == NULL)
                {
                    ctlw_clean_security_object(securityObj);
                    return NULL;
                }
...
char *ctlw_get_server_uri(ctlw_lwm2m_object_t *objectP,
                     uint16_t secObjInstID)
{
    ...
    if (NULL != targetP)
    {
        return ctlw_lwm2m_strdup(targetP->uri);
    }

    return NULL;
}

uint8_t ctlw_set_server_uri(ctlw_lwm2m_object_t *objectP,
                     uint16_t secObjInstID,
                     const char *serverUri)
{
	security_instance_t *targetP = (security_instance_t *)LWM2M_LIST_FIND(objectP->instanceList, secObjInstID);
	...
	if (NULL != targetP)
	{
		...
		targetP->uri = ctlw_lwm2m_strdup(serverUri);
		if(targetP->uri == NULL)
		{
			return COAP_500_INTERNAL_SERVER_ERROR;
		}
