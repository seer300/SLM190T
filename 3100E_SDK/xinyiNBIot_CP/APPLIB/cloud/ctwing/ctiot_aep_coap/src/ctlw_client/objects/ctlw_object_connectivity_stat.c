...
static uint8_t prv_set_tlv(lwm2m_data_t *dataP, conn_s_data_t *connStDataP)
{
    ...
    case RES_O_SMS_TX_COUNTER:
        ctlw_lwm2m_data_encode_int(connStDataP->smsTxCounter, dataP);
        return COAP_205_CONTENT;
        //break;
    case RES_O_SMS_RX_COUNTER:
        ctlw_lwm2m_data_encode_int(connStDataP->smsRxCounter, dataP);
        return COAP_205_CONTENT;
        //break;
    case RES_O_TX_DATA:
        ctlw_lwm2m_data_encode_int(connStDataP->txDataByte / 1024, dataP);
        return COAP_205_CONTENT;
        //break;
    case RES_O_RX_DATA:
        ctlw_lwm2m_data_encode_int(connStDataP->rxDataByte / 1024, dataP);
        return COAP_205_CONTENT;
        //break;
    case RES_O_MAX_MESSAGE_SIZE:
        ctlw_lwm2m_data_encode_int(connStDataP->maxMessageSize, dataP);
        return COAP_205_CONTENT;
        //break;
    case RES_O_AVERAGE_MESSAGE_SIZE:
        ctlw_lwm2m_data_encode_int(connStDataP->avrMessageSize, dataP);
        return COAP_205_CONTENT;
        //break;
    default:
...
static uint8_t prv_read(uint16_t instanceId, int *numDataP, lwm2m_data_t **dataArrayP, ctlw_lwm2m_object_t *objectP)
{
    uint8_t result;
    int i;
...
static void prv_resetCounter(ctlw_lwm2m_object_t *objectP, bool start)
{
...
static uint8_t prv_exec(uint16_t instanceId, uint16_t resourceId,
                        uint8_t *buffer, int length, ctlw_lwm2m_object_t *objectP)
{
    // this is a single instance object
    if (instanceId != 0)
    {
...
void ctlw_conn_s_updateTxStatistic(ctlw_lwm2m_object_t *objectP, uint16_t txDataByte, bool smsBased)
{
    conn_s_data_t *myData = (conn_s_data_t *)(objectP->userData);
...
void ctlw_conn_s_updateRxStatistic(ctlw_lwm2m_object_t *objectP, uint16_t rxDataByte, bool smsBased)
{
    if (objectP->objID != LWM2M_CONN_STATS_OBJECT_ID)
    {
        ctiot_log_debug(LOG_COMMON_MODULE,LOG_OTHER_CLASS,"input object err, please check object!\r\n");
        return;
    }
...
ctlw_lwm2m_object_t *ctlw_get_object_conn_s(void)
{
    /*
     * The ctlw_get_object_conn_s() function create the object itself and return
    ...
    if (NULL != connObj)
    {
        ...
        connObj->instanceList = ctlw_lwm2m_malloc(sizeof(lwm2m_list_t));
        ...
        connObj->userData = ctlw_lwm2m_malloc(sizeof(conn_s_data_t));
