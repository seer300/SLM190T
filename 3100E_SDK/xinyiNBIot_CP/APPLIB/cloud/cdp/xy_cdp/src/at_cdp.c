...
void at_send_NSMI(int state,uint8_t seq_num)
{
	if(g_cdp_session_info->cdp_nsmi == 1)
	{
		...
		if(state==0)
		{
			sprintf(rsp_cmd,"+NSMI:SENT");
		}	
		else
		{
			if(seq_num == 0)
				sprintf(rsp_cmd,"+NSMI:DISCARDED");
			else
				sprintf(rsp_cmd,"+NSMI:DISCARDED,%d",seq_num);
		}
...
int at_NMGR_req(char *at_buf, char **prsp_cmd)
{
    ...
    if (data_len > 0)
    {
        *prsp_cmd = xy_malloc2(20 + data_len*2);
		if(*prsp_cmd == NULL)
		{
			xy_free(data);
			return  ATERR_NO_MEM;
...
{	
	...
	else if(g_req_type == AT_CMD_QUERY)
	{
		*prsp_cmd = xy_malloc(40);
		snprintf(*prsp_cmd, 40, "+NNMI:%d", g_cdp_session_info->cdp_nnmi);
	}
	else
		return  (ATERR_PARAM_INVALID);
	return AT_END;
...
int at_NMSTATUS_req(char *at_buf, char **prsp_cmd)
{
	...
	*prsp_cmd = xy_malloc(310);
	if(g_req_type == AT_CMD_QUERY)
	{
		if(strcmp((const char *)get_cdp_server_ip_addr_str(), "") == 0)
		{
			snprintf(*prsp_cmd, 96, "+NMSTATUS:NO_UE_IP");
			return AT_END;
		}
		...
		if(g_phandle != NULL)
		{
			...
			{
			snprintf(*prsp_cmd, 96, "+NMSTATUS:INIITIALISING");
			return AT_END;
			}
			else if(context->state == STATE_REGISTERING)
			{
			snprintf(*prsp_cmd, 96, "+NMSTATUS:REGISTERING");
			return AT_END;
			}
		}

		if(g_cdp_session_info->cdp_lwm2m_event_status == ATINY_REG_OK)
			snprintf(*prsp_cmd, 96, "+NMSTATUS:REGISTERED");
	#if VER_BC95
		else if(g_cdp_session_info->cdp_lwm2m_event_status == ATINY_DEREG)
			snprintf(*prsp_cmd, 96, "+NMSTATUS:DEREGISTERED");
		else if(g_cdp_session_info->cdp_lwm2m_event_status == ATINY_REG_FAIL)
			snprintf(*prsp_cmd, 96, "+NMSTATUS:REG_FAILED");
		else if(g_cdp_session_info->cdp_lwm2m_event_status == 10) //服务器拒绝
			snprintf(*prsp_cmd, 96, "+NMSTATUS:REJECTED_BY_SERVER");
	#endif
		else if(g_cdp_session_info->cdp_lwm2m_event_status == 255)
			snprintf(*prsp_cmd, 96, "+NMSTATUS:UNINITIALISED");
		else
		{
	#if VER_BC95
			snprintf(*prsp_cmd, 96, "+NMSTATUS:MO_DATA_ENABLED");
	#else
			snprintf(*prsp_cmd, 96, "+NMSTATUS:REGISTERED");
	#endif
		}
...
int at_QLWULDATA_req(char *at_buf, char **prsp_cmd)
{
	...
	if (at_parse_param("%l(1-1024),%h,%d[0-255]", at_buf, &len,src_data, &seq_num) != AT_OK)
	{
		xy_free(src_data);
		return  (ATERR_PARAM_INVALID);
	}
...
#if VER_BC95
	if(!cdp_send_data(src_data,len,cdp_NON,(uint8_t)seq_num))
#else
	if(!cdp_send_data(src_data,len,cdp_CON,(uint8_t)seq_num))
#endif
	{
		xy_free(src_data);
		return  (ATERR_NOT_ALLOWED);
...
int at_QLWULDATASTATUS_req(char *at_buf, char **prsp_cmd)
{
    ...
    *prsp_cmd = xy_malloc(36);
	...
	if(seq_num != 0 && cdp_find_match_udp_node(seq_num))
	{
       	 sprintf(*prsp_cmd, "+QLWULDATASTATUS:0,%d",seq_num);
		 return AT_END;
	}
	
	if(!seq_num)
    	sprintf(*prsp_cmd, "+QLWULDATASTATUS:%d", g_send_status);
	else
		sprintf(*prsp_cmd, "+QLWULDATASTATUS:%d,%d", g_send_status,seq_num);
    return AT_END;
}
