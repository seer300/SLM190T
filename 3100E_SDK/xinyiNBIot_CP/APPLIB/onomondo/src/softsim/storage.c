...
struct ss_buf *ss_storage_read_file(const struct ss_list *path, size_t read_offset, size_t read_len)
{
	...
	if (rc != 0) {
		...
		SS_LOGP(SSTORAGE, LERROR, "unable to seek (read_offset=%d) requested data in content file: %s\n",
	...
	if (fgets_rc != read_len) {
		...
		SS_LOGP(SSTORAGE, LERROR, "unable to load content (read_offset=%d, read_len=%lu) from file: %s\n", read_offset, read_len, host_path);
		SS_FREE(line_buf);
		ss_fclose(fd);
		return NULL;
	}
...
int ss_storage_update_def(const struct ss_list *path)
{
	...
	// 	}
	// }
	fwrite_rc = ss_fwrite(file->fci->data, 1, file->fci->len, fd);
