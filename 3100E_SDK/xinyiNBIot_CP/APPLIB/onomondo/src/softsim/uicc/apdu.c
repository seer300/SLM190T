...
void ss_apdu_parse_exhaustive(struct ss_apdu *apdu, uint8_t *buffer, size_t len)
{
	...
	if (len == HEADER_SIZE + 1) {
		...
		if (apdu->hdr.ins != 0x20 && apdu->hdr.ins != 0x70 && apdu->hdr.ins != 0x2C && apdu->hdr.ins != 0xf2 && apdu->hdr.ins != 0x2f)
			le = le == 0 ? 256 : le; // Verify pin command gets in here with an Lc value of 0...
