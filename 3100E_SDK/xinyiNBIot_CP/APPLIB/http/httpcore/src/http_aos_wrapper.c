...
void *http_calloc(size_t n, size_t size)
{
    void *p = http_malloc(n * size);
...
int http_tcp_conn_wrapper(httpclient_t *client, const char *host)
{
	...
	/* Create TCP socket */
	if ((client->socket = xy_socket_by_host(host, Sock_IPv46, IPPROTO_TCP, 0, client->remote_port, NULL)) == -1) {
		ret = HTTP_ECONN;
	}
	else {
		ret = 0;
...
int http_tcp_recv_wrapper(httpclient_t *client, char *buf, int buflen, int timeout_ms, int *p_read_len)
{
    ...
    while (readLen < buflen) {
        ...
        if (select_ret > 0) {
            ...
            http_info("assume select timeouted here");
            ret = HTTP_ETIMEOUT;
        } else {
        ...
        if (ret > 0) {
            readLen += ret;
        } else if (ret == HTTP_ECLSD || ret == HTTP_ETIMEOUT) {
            break;
        } else {
    ...
    *p_read_len = readLen;
    if (ret == HTTP_ECLSD || ret == HTTP_ETIMEOUT) {
        return ret;
    } else {
        return HTTP_SUCCESS;
    }
...
int http_ssl_conn_wrapper(httpclient_t *client, const char *host)
{
    ...
    client->ssl = (httpclient_ssl_t *)http_calloc(1, sizeof(httpclient_ssl_t));
...
#endif
        ret = mbedtls_pk_parse_key(&ssl->pkey, (const unsigned char *)client->client_pk, client->client_pk_len, NULL, 0, NULL, NULL);
        if (ret != 0) {
