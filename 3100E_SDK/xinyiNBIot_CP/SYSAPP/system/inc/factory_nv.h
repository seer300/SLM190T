...
#define NVM_MAX_CANDIDATE_FREQ_NUM         G_LRRC_SUPPORT_BAND_MAX_NUM
...
#define LPM_TCMCNT_NUM   (120/2+1)

struct tcmcnt_table_t
{
    unsigned short tcmcnt_table[LPM_TCMCNT_NUM];
    unsigned long pll_divn_table[LPM_TCMCNT_NUM];
...
typedef struct
{
    ...
    unsigned char    ucQicsgpFlg:1;                                         //##0##,use to record weather cid is seted by AT+QCICSGP
    unsigned char    ucPadding6:5;
...
typedef struct
{
    unsigned char                      ucUserSetFlg:1;                      //##0## 1:use  0:not use
    unsigned char                      ucPdpTypeFlag:1;                   //##0##
    unsigned char                      ucApnLen:6;                        //##0##
    unsigned char                      ucPdpType:3;                       //##0## 1:IPv4; 2:IPV6; 3:IPV4V6
    //############################ QUECTEL ######################################
    unsigned char                      ucUsedFlg:1;                       //##0##  1:use  0:not use
    unsigned char                      ucH_comp:1;                        //##0##
    unsigned char                      ucSecurePco:1;                     //##0##
    unsigned char                      ucNSLPI:1;                         //##0##
    //###########################################################################
...
typedef struct
{
    ...
    LNB_NAS_USER_SET_DEFAULT_CID_INFO tInitDefCidInfo;                      //inital default cid info
    ...
    unsigned char    ucBC25Flg:1;                                           //##0##,BC25
    ...
    unsigned char    ucRaiRelExcepIgnoreFlg:1;                              //##0##,1 for quectel

    unsigned char    ucCopsMode:3;                                          //##0##
    unsigned char    ucNpbplmnsVal:1;                                       //##0##
    unsigned char    ucNbackoffVal:1;                                       //##0##
    unsigned char    ucNPlmnsOosIndFlg:1;                                   //##0##
    unsigned char    ucRefreshRestartPsFlg:1;                               //##0##
    unsigned char    ucPowOnATHandle:1;                                     //##0##,1:AT+NBAND,can use in poweroff
    
    unsigned short   usTDelayLen_S;                                         //##180##&&
    unsigned char    bXtalTrackDisable:1;                                   //whether to enable crystal aging tracking,.default 0(enable)
    unsigned char    bAgcGainJumpDisable:1;                                 //whether to enable agc gain jump when encounter strong interference(i.e. DC problem),default 0(enable)
    unsigned char    bMaxTxPowerEnhEnable:1;                                //whether to increase tx power additioally when confgiuring 23dBm 0:disable(default);1:enbale;
    unsigned char    ucMaxTxPowerRsrpOffset:5;                              //only enable txPowerEnh when  rsrp is larger than -126dBm+ucMaxTxPowerRsrpOffset; unit:1dB max=2^5-1=31
    unsigned char    ucSrchBandMaxNum;                                      //##0##
    unsigned char    ucSimulatorUiccFlg:1;
    unsigned char    ucSimulatorCardNum:4;
    unsigned char    ucSimulatorSubCardNum:2;
    unsigned char    ucRpmSupportFlag:1;                                    //##0##
    unsigned char    aucSimuuiccImsi[NAS_IMSI_LTH];
    unsigned char    aucSimuuiccAuthKey[NAS_AUTH_KEY_LTH];                  //##000102030405060708090A0B0C0D0E0F##
    unsigned char    aucSimuuiccAuthOp[NAS_AUTH_OP_LTH];                    //##34343434343434343434343434343434##
	unsigned char    uc8821OtaTest;
    unsigned char    ucQlockeranoffset;                                      //##0##
    unsigned char    ucCfun0RstFlg:1;                                        //##0##
    unsigned char    ucQspchscMode:1;                                       //##0##
    unsigned char    ucDwellRptFlg:1;                                        //##0## whether to report freq and sys result.for quectel
...
typedef struct
{
	...
	uint32_t reserved[2];
	//rf_rx_dc_t		rflb_rx_dc;
	//rf_rx_dc_t		rfhb_rx_dc;
}rf_dc_iq_t;
...
typedef struct
{
	uint8_t hib_enable;	   // ##1## ble_hibernate休眠,0:只能进lpm，该状态下广播，可唤醒NB；1：休眠，不广播，不能唤醒NB
	uint8_t pairing_mode;  // ##0## 配对模式 0:不加密;1:加密，用户不需要操作;2：加密，需要手机输入验证码，支持MITM;	   3:SC属性加密，用户不需要操作;4：SC属性加密，需要手机输入验证码，支持MITM;
	uint8_t key_mode;	   // ##0## 0:随机密钥；1:固定密钥，只有在ble_pairing_mode为2或者4时生效
	uint8_t rf_power;	   // ##3## 蓝牙发射功率0:-20dB  1:-5dB  2:-3dB  3:0dB  4:3dB  5:5db  6:6dB  7:7dB  8:10dB  9:-30dB

	uint32_t passkey; 	   // ##123456##&& 配对验证码，只有在ble_pairing_mode为2或者4时生效。密码仅可配置为六位数字。初始配对密码（<passkey> 初始值）为IMEI号的后六位。

	uint16_t mtusize;	   // ##512##蓝牙之间单包数据最大长度
	uint8_t freq_offset;   // ##23## 蓝牙频偏，客户硬件调试确定好最佳值后通过NV Edit工具修改。
	...
	uint8_t ble_name[30];  // ##XY_1200E## 蓝牙设备名称，最大长度为29字节。初始蓝牙名称（<device_name> 的初始值）为IMEI号的后六位。自动保存至 NVRAM；深睡眠唤醒后有效。
	uint8_t ble_name_adv;  // ##0## ble设置广播后不能设置名称
	uint8_t padding1;

	uint16_t advt_min;     // ##160## 蓝牙广播周期最小值。32 - 16448
	uint16_t advt_max;     // ##160## 蓝牙广播周期最大值。32 - 16448

	uint8_t accessmode;    // ##0## 连接成功不进入透传模式，1：连接成功自动进入透传模式
    uint8_t connecturc;    // ##0## 0：连接成功没有URC上报，1：连接成功有URC上报
    uint8_t broadcast;     // ##0## 0：关闭广播，1：打开广播
    uint8_t padding2;
    
    uint8_t ble_mac[6];    // ##0## 蓝牙设备地址,十六进制数值。
	uint8_t reserved[15];
} BLE_cfg_t;
...
typedef struct
{
    ...
    uint8_t  off_debug;             //##0##&& 产线上需配为1。CP核的软件debug开关，值为0时，assert及看门狗异常不会自动重启

    uint8_t  deepsleep_delay;       //##30##&& 非RTC唤醒或有外部AT命令输入，系统延迟若干秒后方容许尝试进深睡。若设为0，则必须由用户输入"AT+WORKLOCK=0"方可以进入深睡
    ...
    uint8_t  dump_mem_into_flash;   //##1##&& CP核软件死机时，保存现场到flash的FOTA备份区；OPENCPU产品该值不生效，需要AP核用户主动调用dump_into_to_flash
    ...
    uint8_t  log_rate;              //##0##&& log口速率，默认为0，OPENCPU最高可选5。0:921600 1:9600 2:19200 3:38400 4:57600 5:115200 6:380400 7:460800

    uint8_t  padding2;              //
    uint8_t  sim_type;              //##1## 1,使用物理SIM卡；0，使用软SIM，暂未使用
...
#if GNSS_EN
    uint8_t gnss_cold_start;    //##0## GNSS冷启动标志，0：默认设置禁止冷启动，使用热启动，1：使用冷启动
    uint8_t gnss_padding1;
...
typedef struct {
    ...
    softap_fac_nv_t  softap_fac_nv;   //begin from 0X680,platform factory nv,have used 160 bytes

}factory_nv_t;


#define RAM_FACTORY_NV_LEN                sizeof(factory_nv_t)
extern factory_nv_t *g_factory_nv;

typedef struct {
    volatile unsigned int rf_mt_mode;
    volatile unsigned int tx_low_power_flag;
    volatile unsigned int tx_multi_tone_power_flag;
    volatile unsigned int xtal_switch;
    volatile unsigned int xtal_type;
    volatile int initXoPPMValue;//100 means 1PPM, loc- ref
    //volatile double lowTemperaFactor;
    //volatile double highTemperaFactor;
    volatile unsigned short  tSensorValue15;
    volatile unsigned short  tSensorValue22;
    volatile unsigned short  code_rc;

    unsigned short  rcFlag;
    volatile unsigned int rfVbatFlag;
    volatile unsigned int    xtalCLK;
    volatile unsigned int    bbpllDivN;
    volatile unsigned int    bbpllFracN;
} rf_normal_mode_t;
...
extern void psNvImeiSnSave();

extern rf_normal_mode_t g_rf_mt;
