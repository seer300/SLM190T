...
/***************************************SRAM CP***********************************************************
           _ _ _ _ _ _ _ ____________0x10000000
            |           |
        160K|  0x28000  | text/data/bss/heap/stack
           _|_ _ _ _ _ _|____________0x10028000

***************************************************************************************************/
#define  SRAM_BASE    0x10000000
#define  SRAM_LENGTH  0x00028000
/*********************************SRAM AP*************************************************************
              _ _ _ _ _ _ ____________0x20000000
            |           |
            |  0x1F200  | AP TEXT/DATA/BSS/HEAP/STACK
            |_ _ _ _ _ _|____________0x2001FF80
***************************************************************************************************/
#define  AP_SRAM_BASE          0x20000000
#define  AP_SRAM_LENGTH        0x00020000

#define  AP_SRAM_DUMP_DATA_LEN   (0xBB80)
#define  AP_SRAM_DUMP_DATA_BASE  (0x20000000 + 0x10000 - AP_SRAM_DUMP_DATA_LEN)
...
#define  BAK_MEM_OTP_PADDING                    (BAK_MEM_OTP_RCCAL_BASE + BAK_MEM_OTP_RCCAL_LEN)         // 0x60011F0C,OTP预留空间剩余部分，截止地址为0x60011FF0
#define  BAK_MEM_OTP_PADDING_LEN                228  
...
#define  BAK_MEM_CP_USED_APRAM_SIZE       (BAK_MEM_SHM_BASE + 0x18)             // 0x60010018,单字节，未使用！CP向AP借的heap大小
...
#define  BAK_MEM_SOC_VER                  (BAK_MEM_SHM_BASE + 0x4a)             // 0x6001004a,单字节，二级boot通过OTP识别芯片类型，AP核软件平台使用。0:XY1200L;1:XY1200SL;2:XY2100SL;3:XY1200;4:XY1200S;5:XY2100S;6:1200E;7:1200E+
...
#define  BAK_MEM_LPM_NV_WRITE_BUFF        (BAK_MEM_SHM_BASE + 0x74)             // 0x60010074，64个字节
...
/**************************************FLASH 2M********************************************
             ...
             _ _ _ _ _ _ _ __________0x0000_0000
            |           |
            |   0x1000  |  CP boot
            |_ _ _ _ _ _|____________0x0000_1000
            |           |                                 //secondary bootloader prime header
            |   0x3000  |  data for AP 1st boot           //secondary bootloader backup header
            |_ _ _ _ _ _|____________0x0000_4000          //image_info header
            |           |
            |   0x1000  |  security boot header
            |_ _ _ _ _ _|____________0x0000_5000
            |           |
            |   0x1000  |  FOTA_BREAKPOINT_INFO          //FOTA断电续传续升相关状态机信息
            |_ _ _ _ _ _|____________0x0000_6000
            |           |
            |   0x7000  |  AP secondary boot prime
            |_ _ _ _ _ _|____________0x0000_D000
            |           |
            |   0x7000  |  AP secondary boot backup
            |_ _ _ _ _ _|____________0x0001_4000
            |           |
            |   0x2000  |  RF calibration NV              //射频校准NV，升级时不擦除；烧录时根据工具配置是否重新擦写
            |_ _ _ _ _ _|____________0x0001_6000
            |           |
            |   0x1000  |  factory NV                     //原始出厂NV，版本烧录时写入，永不修改；FOTA升级根据需要可重写入
            |_ _ _ _ _ _|____________0x0001_7000
            |           |
            |  0x15D000 |  CP(text/rodata)               //暂时不知道1级boot校验值的计算方式，等知道后将Reserved合并
            |_ _ _ _ _ _|____________0x0017_4000
            |           |
            |  0x6F000  |  AP(text/rodata/FOTA packet)   //FOTA差分包保存的首地址决定于AP版本大小；参见BAK_MEM_FOTA_FLASH_BASE
            |_ _ _ _ _ _|____________0x001E_3000
            |           |
            |   0x2000  |  CP NON-volatile NV            //非易变NV,升级或烧录时擦除;PsUpdateInvar里申请内存,Ps_ftl_read里读取
            |_ _ _ _ _ _|____________0x001E_5000
            |           |
            |   0x1000  |  working factory NV            //工作态的出厂NV，运行时可修改;FOTA升级时，此处采用增量升级方式
            |_ _ _ _ _ _|____________0x001E_6000
            |           |
            |   0x1000  |  32K CALI                      //32K校准数据分区
            |_ _ _ _ _ _|____________0x001E_7000
            |           |
            |   0x9000  |  LittleFS Region               //文件系统分区
            |_ _ _ _ _ _|____________0x001F_0000
            |           |
            |   0x10000 |  user NV                       //该区域供用户保存私有数据，FOTA升级时不擦除，不升级
            |_ _ _ _ _ _|____________0x0020_0000
...
#define SECONDBOOT_PRIME_HEADER_BASE  0x00001000  /*二级boot头部信息，FOTA升级过程的状态机信息*/
#define SECONDBOOT_BACKUP_HEADER_BASE 0x00002000

#define  IMAGE_INFO_ADDR          0x00003000

#define  IMAGE_CHECKSUM_ADDR      (0x00004000)
#define  IMAGE_CHECKSUM_SECTION_LEN 0x1000

#define FOTA_BREAKPOINT_INFO_ADDR    (0x00005000)  /*FOTA下载过程中存储下载状态，已接受数据长度等信息，用于断电续传或续升*/
#define FOTA_BREAKPOINT_INFO_LEN     (0x1000)


#define  NV_FLASH_RF_BASE         0x00014000
#define  NV_FLASH_RF_BAKUP_BASE   0x00015000
#define  NV_FLASH_RF_SIZE         0x1000

#define  NV_MAIN_FACTORY_BASE     0x00016000            //原始出厂NV，版本烧录时写入，永不修改；FOTA升级根据需要可重写入

#define  CP_CODE_START_ADDR		  0x00017000
#define  CP_CODE_LENGTH			  0x15D000

#define  NV_FLASH_FACTORY_BASE    0x001E5000             //工作态的出厂NV，运行时可修改;FOTA升级时，若需要升级原始出厂NV，则会擦除此处；否则不擦除
#define  NV_FLASH_FACTORY_LEN     0x1000

/*依次存放AP核版本及FOTA备份区，其中尾部用来保存动态运行的一些信息，FOTA升级时可擦除*/
#define  ARM_FLASH_BASE_ADDR      0x00174000             // AP核使用的flash开始地址
#define  ARM_FLASH_BASE_LEN       0x6F000                // AP核中text/data/fota共用的flash总长度

/*死机回写flash的信息内容，与FOTA区域复用。FOTA升级会擦除,从尾部向头部存放*/
#define  DUMP_INFO_BASE_ADDR     (ARM_FLASH_BASE_ADDR + ARM_FLASH_BASE_LEN - 0X8000)  //0x001DB000
#define  DUMP_INFO_BASE_LEN      0X1000 

/*借用FOTA尾部4K,存放运行的debug信息，如fota、IMEI擦除等时刻点。复用FOTA备份区，FOTA升级会擦除*/
#define  RUNINFO_DEBUG_ADDR     (DUMP_INFO_BASE_ADDR + DUMP_INFO_BASE_LEN)   //0x001DC000
#define  RUNINFO_DEBUG_LEN      0X1000

/* 模组形态下，若8K retension内存下电，则回写到FOTA区域的尾部6K flash中，内容通常为易变NV。复用FOTA备份区，FOTA升级会擦除 */
#define  BAK_MEM_FLASH_TOTAL_LEN 0x6000UL
#define  BAK_MEM_FLASH_BASE      (RUNINFO_DEBUG_ADDR + RUNINFO_DEBUG_LEN)  //0x001DD000
#define  BAK_MEM_FLASH_LEN       0x3000UL 
#define  BAK_MEM_FLASH_BASE2     (BAK_MEM_FLASH_BASE+BAK_MEM_FLASH_LEN)    //0x001E0000
#define  BAK_MEM_FLASH_LEN2      0x3000UL  


#define  NV_NON_VOLATILE_BASE     0x001E3000
#define  NV_NON_VOLATILE_LEN      0x2000

#define  CALIB_FREQ_BASE         0x001E6000          //32K校准数据、xy_timer链表、BLE运行态NV
#define  FLASH_AP_TIME_LEN       128                     //sizeof(Cali_Ftl_t)+128+BLE_NVRAM_LENGTH
#define  CALIB_FREQ_LEN          0x1000

#define  FS_FLASH_BASE            0x001E7000            //芯翼文件系统分区
#define  FS_FLASH_LENGTH          0x9000

#define  FLASH_SECTOR_LENGTH      0x1000UL

/* 该区域供用户保存私有数据，FOTA升级时不擦除；用户不得私自更改该区域大小范围，若有需要，请联系芯翼研发!
   若需要磨损功能及断电异常保护能力，可使用xy_flash.h相关接口 */
#define  USER_FLASH_BASE          0x001F0000
...
/***************************************************************************************************
            ...
            |_ _ _ _ _ _|____________0x0001_7000
            |           |
            |  0x200000 |  CP(text/rodata)               //CP版本
            |_ _ _ _ _ _|____________0x0021_7000
            |           |
            |  0x1BC000 |  AP(text/rodata/FOTA packet)   //AP版本与FOTA备份区。FOTA差分包保存的首地址决定于AP版本大小；参见BAK_MEM_FOTA_FLASH_BASE
            |_ _ _ _ _ _|____________0x003D_3000
            |           |
            |   0x2000  |  CP NON-volatile NV             //非易变NV,升级或烧录时擦除；PsUpdateInvar里申请内存,Ps_ftl_read里读取
            |_ _ _ _ _ _|____________0x003D_5000
            |           |
            |   0x1000  |  working factory NV             //工作态的出厂NV，运行时可修改;FOTA升级时，若需要升级原始出厂NV，则会擦除此处；否则不擦除
            |_ _ _ _ _ _|____________0x003D_6000
            |           |
            |   0x1000  |  32K  CALI                      //32K校准数据分区
            |_ _ _ _ _ _|____________0x003D_7000
            |           |
            |   0x9000  |  LittleFS Region                //文件系统分区
            |_ _ _ _ _ _|____________0x003E_0000
            |           |
            |   0x20000 |  user NV                       //该区域供用户保存私有数据，FOTA升级时不擦除，不升级
            |_ _ _ _ _ _|____________0x0040_0000
**********************************************************************************************/
...
#define FOTA_BREAKPOINT_INFO_ADDR    (0x00005000)  /*FOTA下载过程中存储下载状态，已接受数据长度等信息，用于断电续升*/
#define FOTA_BREAKPOINT_INFO_LEN     (0x1000)
...
#define  CP_CODE_START_ADDR		 0x00017000
#define  CP_CODE_LENGTH			    0x200000

#define  NV_FLASH_FACTORY_BASE    0x003D5000             //工作态的出厂NV，运行时可修改;FOTA升级时，若需要升级原始出厂NV，则会擦除此处；否则不擦除 
#define  NV_FLASH_FACTORY_LEN     0x1000

/*依次存放AP核版本及FOTA备份区，其中尾部用来保存动态运行的一些信息，FOTA升级时可擦除*/
#define  ARM_FLASH_BASE_ADDR      0x00217000               
#define  ARM_FLASH_BASE_LEN       0x1BC000

/*死机回写flash的信息内容，与FOTA区域复用。FOTA升级会擦除*/
#define  DUMP_INFO_BASE_ADDR     (ARM_FLASH_BASE_ADDR + ARM_FLASH_BASE_LEN - 0X8000)  //0x003CB000
#define  DUMP_INFO_BASE_LEN      0X1000 

/*借用FOTA尾部4K,存放运行的debug信息，如fota、IMEI擦除等时刻点。复用FOTA备份区，FOTA升级会擦除*/
#define  RUNINFO_DEBUG_ADDR      (DUMP_INFO_BASE_ADDR + DUMP_INFO_BASE_LEN)   //0x003CC000
#define  RUNINFO_DEBUG_LEN       0X1000

/* 模组形态下，若8K retension内存下电，则回写到FOTA区域的尾部6K flash中，内容通常为易变NV。复用FOTA备份区，FOTA升级会擦除 */
#define  BAK_MEM_FLASH_TOTAL_LEN  0x6000UL
#define  BAK_MEM_FLASH_BASE       (RUNINFO_DEBUG_ADDR + RUNINFO_DEBUG_LEN)  //0x003CD000
#define  BAK_MEM_FLASH_LEN        0x3000UL 
#define  BAK_MEM_FLASH_BASE2      (BAK_MEM_FLASH_BASE+BAK_MEM_FLASH_LEN)    //0x003D0000
#define  BAK_MEM_FLASH_LEN2       0x3000UL 


#define  NV_NON_VOLATILE_BASE     0x003D3000
#define  NV_NON_VOLATILE_LEN      0x2000

#define  CALIB_FREQ_BASE         0x003D6000              //32K校准数据、xy_timer链表、BLE运行态NV
#define  FLASH_AP_TIME_LEN       128                     //sizeof(Cali_Ftl_t)+128+BLE_NVRAM_LENGTH
#define  CALIB_FREQ_LEN          0x1000

#define  FS_FLASH_BASE            0x003D7000             //芯翼文件系统分区
#define  FS_FLASH_LENGTH          0x9000
...
#define  USER_FLASH_BASE          0x003E0000
